...
define([
    ...
    $.widget('mage.myOrdersFilter', {
        ...
        bindOpenCloseListenerToFilterButton: function () {
            ...
            this.els.filterShowBtn.on("click", function () {
                this.els.extraFilters.show();
                this.els.filterCloseBtn.show();
                if (typeof window.checkout !== 'undefined' && !window.checkout.is_sde_store && (window.checkout.is_fcl_customer || window.checkout.is_commercial)) {
            ...
            this.els.filterCloseBtn.on("click", function () {
        ...
        createFilterSummary: function () {
            ...
            this.$form.children('.fieldset').find('.field').each(function () {
                var $field = $(this),
                ...
                filters = filters.concat(filledInputs.map(function (filledInput) {
                    ...
                    if (filledInput.nodeName === 'SELECT') {
                        ...
                        // B-1213999 - "View Order" for completed should redirect to Order History with only shipped, ready for pickup, or complete
                        // D-95025
                    } else if($filledInput.attr('id') == 'custom-order-status' && 
                        $('#custom-order-status').val() == 'shipped;ready_for_pickup;complete'){
                        value = 'Shipped, Ready For Pickup, Delivered';
                        label = 'Status';
                        $('.reset-btn').prop("disabled", false);                     
                    } else { // use value of input field verbatim
						value = filledInput.value;
						
						// B-1145903 - add condition to remove value from hidden field
                        if(filledInput.type != undefined && filledInput.type == 'hidden'){
							value = '';
						}
                    }
            ...
            filters.forEach(function (filter) {
                ...
                // D-98045 - RT-ECVS-Filter for order total is coming twice
                // D-158119 - Apply the fix for selfreg and sde stores as well.
                if ((filter.name == 'order-total-min' || filter.name == 'order-total-max') && filter.label == '') {
                    return;
                }
