 ...
 * See COPYING.txt for license details.
 */
class SdeCategory extends \Magento\Framework\App\Action\Action
{
    /**
     * @var array $catResult
     */
    ...
    public function execute()
    {
        try {
            ...
            $sdeRootCatId = $sdeGroupObj->getRootCategoryId();

            $b2bGroupObj = $this->groupFactory->create()->load($b2bStoreCode, 'code');
            $b2bRootCatId = $b2bGroupObj->getRootCategoryId();

            $sdeCatTree = $this->categoryManagementInterface->getTree($sdeRootCatId);
            $sdeChildCategory = $sdeCatTree->getChildrenData();

            if (is_array($sdeChildCategory) && count($sdeChildCategory)) {
                $this->traverseChildren($sdeChildCategory, $b2bRootCatId);
            }

            $updateResult = $this->jsonHelper->jsonEncode($this->catResult);
            $this->logger->info('Ondemand controller hit : Copy SDE Print Product Category : ' . $updateResult);
    ...
    public function copyAndMoveCategory($copyCatId, $parentCatId)
    {
        ...
        if ($category->getName() == 'Print Products') {
            $copy->setName('SDE ' . $category->getName());
        }
        $copy->setUrlKey('sde-' . $category->getUrlKey());
        $copy->save();
    ...
    public function updateCategoryPermission($oldCatId, $newCatId)
    {
        ...
        if ($permissionCollection && $permissionCollection->getSize()) {
