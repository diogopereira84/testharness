...
define([
    ...
    return Select.extend({
        ...
        readonlySections : function(value) {
            ...
            dom.get("textarea[name='product[external_prod]']", function (elem) {
        ...
        addNewFieldsForpod : function() {
            ...
            dom.get("textarea[name='product[external_prod]']", function (elem) {
                ...
                if (jQuery('body').hasClass('mazegeeks_download_catalog_items')) {
                    if (!jQuery(".download-field")[0]){
                        self.downloadFieldForNonConfProd();
                    }   
                }

                if (jQuery('body').hasClass('catalog_mvp_custom_docs')) {
                    if (!jQuery(".custom-doc-field")[0]){
        ...
        customDocFieldForConfProd : function() {
            ...
            if (customizeFields !== "") {
                ...
                let arr = [];
                try {
                    arr = JSON.parse(customizeFields);
                } catch (e) {
                    console.log(e);
                }
        ...
        addEditButton : function() {
            let self = this;
            dom.get("textarea[name='product[external_prod]']", function (elem) {
                if(jQuery("textarea[name='product[external_prod]']").is(":visible") &&  jQuery("input[name='product[pod2_0_editable]']").val() == 1) {
                    let isConfigured = jQuery.trim(jQuery("textarea[name='product[external_prod]']").val());
        ...
        productNameHundredCharacter : function() {
            dom.get('.character_limit_hundred div[data-index="attribute_set_id"] .admin__action-multiselect-text', function (elem) {
        ...
        changeFormSubmitUrl: function (value) {
            ...
            var catalogUpdateUrl = window.location.pathname;
            var attributeSetValue = null;
            var attributeSetName = null;
            var hasCatalogEdit = window.location.toString().includes("catalog/product/edit");
            
            if (catalogUpdateUrl) {
                var parts = catalogUpdateUrl.split('/');
                var setIndex = parts.indexOf('set');
                if (setIndex !== -1 && setIndex + 1 < parts.length || hasCatalogEdit) {
                    attributeSetValue = parts[setIndex + 1];
                    
                    if (hasCatalogEdit) {
                        attributeSetValue = value;
                    }
                    var productAttributeSetCollection =  typeof (window.productAttributeSetCollection) != 'undefined' && window.productAttributeSetCollection != null ? window.productAttributeSetCollection : null;

                    if (productAttributeSetCollection) {
                        var jsonObject = JSON.parse(productAttributeSetCollection);
                        attributeSetName = jsonObject[attributeSetValue] || null;
                    }
                }
            } 
            ...
            if (hasCatalogEdit) {
                ...
                if (!value || !attributeSetName) {
                    attributeSetName = typeof (window.productAttributeSetName) != 'undefined' && window.productAttributeSetName != null ? window.productAttributeSetName : null;
                }
            }
