 ...
 */
class UpdateItemQty extends \Magento\Checkout\Controller\Cart\UpdateItemQty implements HttpPostActionInterface
{
    /**
    ...
    public function __construct(
        ...
        parent::__construct($context, $this->quantityProcessor, $this->formKeyValidator, $this->checkoutSession, $this->json, $this->logger);
    }
        ...
        try {
            ...
            foreach ($cartData as $itemId => $itemInfo) {
                ...
                if ($item) {
                    ...
                    if ($this->helper->isCBBToggleEnabled()) {
                        try {
                            if($this->nonCustomizableProductModel->isProductPunchoutDisabledForThirdPartyItem($item)) {
                                ...
                                $quoteItem = $this->update->updateThirdPartyItemSellerPunchout($item, $item->getProduct(), null, $cartData);
                                $quoteItem->save();
                            }
                        ...
                        try {
                            if($this->nonCustomizableProductModel->isProductPunchoutDisabledForThirdPartyItem($item)) {
                                ...
                                $quoteItem = $this->update->updateThirdPartyItemSellerPunchout($item, $item->getProduct());
                                $quoteItem->save();
                            } else {
                                $this->updateItemQuantity($item, $qty);
                            }
