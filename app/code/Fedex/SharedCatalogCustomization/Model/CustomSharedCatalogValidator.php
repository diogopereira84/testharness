<?php
/**
 * Fedex_SharedCatalogCustomization
 *
 * @category   Fedex
...
class CustomSharedCatalogValidator extends SharedCatalogValidator
{
    private const GROUP_CODE_MAX_LENGTH = 200;
     ...
     * @param GroupRepositoryInterface|null $storeGroupRepository
     */
    public function __construct(
        ...
        $validateStore = false,
        ?GroupRepositoryInterface $storeGroupRepository = null
    ) {
        parent::__construct(
            $sharedCatalogManagement,
            $sharedCatalogCollectionFactory,
            $storeRepository,
    ...
    public function validate(SharedCatalogInterface $sharedCatalog)
    {
        ...
        if ($sharedCatalog->getStoreId()) {
            $this->storeGroupRepository->get($sharedCatalog->getStoreId());
        }

        $this->customValidateSharedCatalog($sharedCatalog);
        $this->validateCustomerGroupChanges($sharedCatalog);
    }

    /**
     * customValidateSharedCatalog function
     *
     * @param SharedCatalogInterface $sharedCatalog
     * @return void
     */
    protected function customValidateSharedCatalog(SharedCatalogInterface $sharedCatalog)
    {
        if (mb_strlen($sharedCatalog->getName()) > self::GROUP_CODE_MAX_LENGTH) {
