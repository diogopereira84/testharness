define([
    ...
    var ProductTranslator = (function () {
        ...
        ProductTranslator.prototype.translateProductInstance = function (data) {
            ...
            this.elementCopy(data, productInstance, { features: true, contentRequirement: true, contentAssociations: true, inserts: true, exceptions: true, addOns: true, properties: true, pageExceptions: true, externalSkus: true, externalProductionDetails: true, contextKeys: true });
        ...
        ProductTranslator.prototype.translateProductData = function (data, controlId) {
            ...
            if (data != null) {
                ...
                productData.setCrossSellProducts(this.translateCrossSellProducts(data.crossSellProducts));
                productData.setUpSellProducts(this.translateUpSellProducts(data.upSellProducts));
            }
            return productData;
        };
        ...
        ProductTranslator.prototype.translateProduct = function (po) {
            ...
            product.setProductType(po.productType);
        ...
        ProductTranslator.prototype.translateCrossSellProducts = function (crossSellProducts) {
            var outCrossSells = new ProductEngine.ArrayList();
            if (crossSellProducts) {
                for (var i = 0; i < crossSellProducts.length; i++) {
                    outCrossSells.add(this.translateProductRelation(crossSellProducts[i]));
                }
            }
            return outCrossSells;
        };
        ProductTranslator.prototype.translateUpSellProducts = function (upSellProducts) {
            var outUpSells = new ProductEngine.ArrayList();
            if (upSellProducts) {
                for (var i = 0; i < upSellProducts.length; i++) {
                    outUpSells.add(this.translateProductRelation(upSellProducts[i]));
                }
            }
            return outUpSells;
        };
        ProductTranslator.prototype.translateProductRelation = function (pr) {
            var productRelation = new ProductEngine.ProductRelation();
            productRelation.setProductId(pr.productId);
            productRelation.setVersion(pr.version);
            productRelation.setUnitQty(pr.unitQty);
        ...
        ProductTranslator.prototype.translateFeature = function (fo) {
            ...
            if (fo.contentReqRefIds) {
                for (var i = 0; i < fo.contentReqRefIds.length; i++) {
                    feature.contentReqRefIds.add(fo.contentReqRefIds[i]);
                }
            }
            return feature;
        };
        ProductTranslator.prototype.translateChoice = function (co) {
            ...
            choice.setSku(co.sku);
            choice.setSkuQty(co.skuQty);
            choice['@type'] = 'Choice';
            if (co.properties) {
            ...
            if (co.contentReqRefIds) {
                for (var i = 0; i < co.contentReqRefIds.length; i++) {
                    choice.contentReqRefIds.add(co.contentReqRefIds[i]);
                }
            }
            return choice;
        };
        ...
        ProductTranslator.prototype.translatePageException = function (inFe) {
            ...
            for (var i = 0; i < inFe.contentReqRefIds.length; i++) {
                outFe.contentReqRefIds.add(inFe.contentReqRefIds[i]);
            }
        ...
        ProductTranslator.prototype.serializeProductRelations = function (prsIn) {
            var prsOut = [];
            var prOut = {};
            var it = prsIn.iterator();
            while (it.hasNext()) {
                prOut = {};
                this.elementCopy(it.next(), prOut);
                prsOut.push(prOut);
            }
            return prsOut;
        };
        ProductTranslator.prototype.serializeProductInstance = function (productIn, supportsPropMap) {
            var productOut = {};
            var inProperty = {};
            this.elementCopy(productIn, productOut, { featureIdMap: true, features: true, properties: true, contentRequirements: true, inserts: true, exceptions: true, addOns: true, contentAssociations: true, category: true, pageExceptions: true, products: true, externalSkus: true, externalProductionDetails: true });
            productOut.features = [];
        ...
        ProductTranslator.prototype.serializeProductionContentAssociations = function (productionContentAssociations) {
            var outPrdContent = {};
            this.elementCopy(productionContentAssociations, outPrdContent, {});
