define(["require","exports","peProductTranslator","peProductEngine"], function (require, exports, product_translator_1, ProductEngine) {
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
var ProductController = (function () {
    function ProductController(productDataURI, productProvider, clientId) {
        ...
        this.allowVendorFeaturesOnly = false;
        ...
        if (productProvider) {
            if (this[productProvider]) {
                this.productProvider = this[productProvider];
            }
        }
    ...
    ProductController.prototype.buildProductServiceProviderRequest = function (productId, productVersion) {
        var productPath = this.productDataURI + '?productId=' + productId;
        if (productVersion != null) {
            productPath += '&version=' + productVersion;
        }
        return productPath;
    };
    ProductController.prototype.defaultProductProvider = function (productId, productVersion, controlId, callBack, errorCallback) {
        ...
        this.resolveProductVersion(productId, productVersion, true, function (result) {
            ProductEngine.Utils.httpGet(_this.buildProductPath(productId, result.version), function (productData, status, headers, config) {
                if (productData != null) {
                    if (controlId) {
                        var keyValue = { controlId: controlId };
                        _this.setProductElementKeyValues(keyValue);
                    }
                    resultProductData.productData = _this.productTranslator.translateProductData(productData, controlId);
                    resultProductData.active = result.active;
                }
                if (callBack) {
                    callBack(resultProductData);
                }
    ...
    ProductController.prototype.productServiceProvider = function (productId, productVersion, controlId, callBack, errorCallback) {
        var _this = this;
        var resultProductData = {
            productData: null,
            active: false
        };
        ProductEngine.Utils.restApiGet(this.buildProductServiceProviderRequest(productId, productVersion), this.clientId, function (responseData, status) {
            if (responseData != null) {
                if (controlId) {
                    var keyValue = { controlId: controlId };
                    _this.setProductElementKeyValues(keyValue);
                }
                resultProductData.productData = _this.productTranslator.translateProductData(responseData.output.configuratorData, controlId);
                resultProductData.active = true;
            }
            if (callBack) {
                callBack(resultProductData);
            }
    ...
    ProductController.prototype.buildProductContext = function () {
        ...
        context.setCoreProduct(this.product);
    ...
    ProductController.prototype.copyProductElements = function (from, to) {
        ...
        while (pitTo.hasNext()) {
            ...
            if (destinationProperty.getName() === 'SYSTEM_SI' && from.getContextKeys().contains('MANUAL_PRODUCTION_NOTE')) {
                if (destinationProperty.getValue() !== null) {
                    destinationProperty.setValue(destinationProperty.getValue() + ' ' + systemSpecialInstructions);
                }
                else {
    ...
    ProductController.prototype.addDisplayableProperties = function (propertiesIterator, targetArray, targetElement) {
        ...
        while (propertiesIterator.hasNext()) {
            ...
            if (input) {
                if (hasSequence === false && input.sequence && input.sequence !== -1) {
                    hasSequence = true;
                }
                ...
                if (property.getBound() != null) {
                    var inBound = property.getBound();
                    displayableProperty.bounds = {
                        min: inBound.min,
                        max: inBound.max,
                        type: inBound.type
                    };
                    ...
                    while (allowedValueIt.hasNext()) {
                        ...
                        if (this.viewOnly) {
                            displayAllowedValue = this.addSelectedAllowedValuesForViewOnly(input.allowedValues, propertyAllowedValue, instanceProperty);
                        }
                        else {
                    ...
                    if (hasSequence === true) {
                        this.sortElements(displayableProperty.allowedValues);
                    }
                }
                else {
                    ...
                    while (boundIt.hasNext()) {
                        ...
                        var allowedValueIt = inBound.allowedValues.iterator();
                        var hasAllwValueSequence = false;
                        while (allowedValueIt.hasNext()) {
                            var propertyAllowedValue = allowedValueIt.next();
                            var displayAllowedValue = null;
                            if (this.viewOnly) {
                            ...
                            if (displayAllowedValue !== null) {
                                if (hasAllwValueSequence === false && displayAllowedValue.sequence && displayAllowedValue.sequence !== -1) {
                                    hasAllwValueSequence = true;
                                }
                                displayableProperty.allowedValues.push(displayAllowedValue);
                            }
                            if (hasAllwValueSequence === true) {
                                this.sortElements(displayableProperty.allowedValues);
                            }
                        }
                    }
            ...
            if (input) {
                ...
                while (boundIt.hasNext()) {
                    ...
                    while (allowedValueIt.hasNext()) {
                        ...
                        if (displayAllowedValue !== null) {
                            if (hasSequence === false && displayAllowedValue.sequence && displayAllowedValue.sequence !== -1) {
                                hasSequence = true;
                            }
                            displayableProperty.allowedValues.push(displayAllowedValue);
                        }
    ...
    ProductController.prototype.addDisplayableFeatures = function (targeElement, configuredElement, parent, displayIdMap) {
        ...
        var selectableVendorChoiceIds = [];
        var selectableVendorFeatureIds = [];
        if (this.vendorCode !== null) {
            if (this.allowVendorFeaturesOnly === true) {
                if (this.productData.designTemplates) {
                    var it_1 = this.productData.designTemplates.iterator();
                    while (it_1.hasNext()) {
                        designTemplate = it_1.next();
                        if (designTemplate.vendorCode === this.vendorCode) {
                            var itfeatureRefs = designTemplate.featureRefs.iterator();
                            var featureRef = null;
                            while (itfeatureRefs.hasNext()) {
                                featureRef = itfeatureRefs.next();
                                selectableVendorChoiceIds.push.apply(selectableVendorChoiceIds, featureRef.choiceIds.values);
                                selectableVendorFeatureIds.push(featureRef.featureId);
                            }
                            break;
                        }
                    }
                ...
                if (this.productData.designTemplates) {
                    var it_2 = this.productData.designTemplates.iterator();
                    while (it_2.hasNext()) {
                        designTemplate = it_2.next();
                        if (designTemplate.vendorCode === this.vendorCode) {
                            var itTemplates = designTemplate.templates.iterator();
                            var template = null;
                            while (itTemplates.hasNext()) {
                                template = itTemplates.next();
                                if (template.id === this.templateId) {
                                    selectableVendorChoiceIds.push.apply(selectableVendorChoiceIds, template.choiceIds.values);
                                    break;
                                }
                            }
                        }
            ...
            var vendorTemplateProperty = this.getPropertyByName('TEMPLATE_VENDOR_CODE', this.selectedProduct.properties);
            if (vendorTemplateProperty != null && vendorTemplateProperty.value !== null && this.productData.designTemplates) {
                var it_3 = this.productData.designTemplates.iterator();
                while (it_3.hasNext()) {
                    designTemplate = it_3.next();
                    if (designTemplate.vendorCode === vendorTemplateProperty.value) {
                        var itFeatureRefs = designTemplate.featureRefs.iterator();
                        while (itFeatureRefs.hasNext()) {
                            var featureRef = itFeatureRefs.next();
                            selectableVendorFeatureIds.push(featureRef.featureId);
                        }
                        break;
                    }
                }
                for (var _i = 0, selectableVendorFeatureIds_1 = selectableVendorFeatureIds; _i < selectableVendorFeatureIds_1.length; _i++) {
                    var featureId = selectableVendorFeatureIds_1[_i];
                    var selectedChoice = this.getSelectedChoiceForFeatureId(featureId, this.selectedProduct);
                    if (selectedChoice) {
                        selectableVendorChoiceIds.push(selectedChoice.id);
                    }
                }
                selectableVendorFeatureIds.length = 0;
            }
        }
        parent.features = [];
        ...
        while (featureIt.hasNext()) {
            ...
            if (selectableVendorFeatureIds.length > 0 && selectableVendorFeatureIds.indexOf(displayableFeature.id) === -1) {
                displayableFeature.selectable = false;
            }
            featureDisplayDetails = this.getDisplayDetailByRefId(feature.id);
            if (featureDisplayDetails != null) {
                if (hasSequence === false && featureDisplayDetails.sequence && featureDisplayDetails.sequence !== -1) {
                    hasSequence = true;
                }
                ...
                while (choiceIt.hasNext()) {
                    ...
                    displayableChoice = { properties: [], selected: (this.isChoiceSelected(choice, targeElement)) };
                    ...
                    if (selectableVendorChoiceIds.length > 0 && this.containsAnyChoice(feature, selectableVendorChoiceIds) && selectableVendorChoiceIds.indexOf(displayableChoice.id) === -1) {
                        displayableChoice.selectable = false;
                    }
                }
                if (hasSequence === true) {
                    if (displayableFeature.choiceGroups) {
                        this.sortGroups(displayableFeature.choiceGroups, 'choices', 'choiceGroups');
                    }
                    if (displayableFeature.choices) {
                        this.sortElements(displayableFeature.choices);
                    }
                }
            }
        }
        if (hasSequence === true) {
            if (parent.featureGroups) {
                this.sortGroups(parent.featureGroups, 'features', 'featureGroups');
            }
            if (parent.features) {
                this.sortElements(parent.features);
            }
        }
    };
    ProductController.prototype.sortGroups = function (groups, elementArrayName, groupArrayName) {
        var _this = this;
        this.sortElements(groups);
        groups.forEach(function (group) {
            if (group[elementArrayName]) {
                _this.sortElements(group[elementArrayName]);
            }
            if (group[groupArrayName]) {
                _this.sortGroups(group[groupArrayName], elementArrayName, groupArrayName);
            }
        });
    };
    ProductController.prototype.sortElements = function (elements) {
        elements.sort(function (a, b) { return a.sequence - b.sequence; });
    };
    ...
    ProductController.prototype.getProductPresetById = function (presetId) {
        ...
        if (this.productData.presets) {
            var it_4 = this.productData.presets.iterator();
            while (it_4.hasNext()) {
                preset = it_4.next();
    ...
    ProductController.prototype.getProductPresetName = function (presetId) {
        if (presetId) {
            if (this.productData.presets) {
                var it_5 = this.productData.presets.iterator();
                while (it_5.hasNext()) {
                    var preset = it_5.next();
                    if (preset.id === presetId) {
                        return preset.name;
                    }
                }
    ...
    ProductController.prototype.selectProductById = function (productId, productVersion, controlId, successCallback, errorCallback) {
        var _this = this;
        this.fetchProductDataById(productId, productVersion, controlId, function (resultProductData) {
            _this.productData = resultProductData.productData;
            _this.selectProduct(_this.productData.product);
            var alert = {
                code: null,
                message: null,
                alertType: null
            };
            if (resultProductData.active === false) {
    ...
    ProductController.prototype.selectProductByHint = function (productHint, successCallback, errorCallback) {
        ...
        this.vendorCode = productHint.templateVendorCode;
        this.templateId = productHint.templateId;
        ...
        this.fetchProductDataById(deserializedHint.productId, deserializedHint.version, deserializedHint.controlId, function (resultProductData) {
            _this.productData = resultProductData.productData;
            _this.product = _this.productData.product;
            _this.selectedProduct = _this.productEngine.buildProductInstance(_this.product);
            _this.selectedProductModified();
            if (_this.vendorCode !== null) {
                if (_this.productData.designTemplates) {
                    var it_6 = _this.productData.designTemplates.iterator();
                    var designTemplate = null;
                    while (it_6.hasNext()) {
                        designTemplate = it_6.next();
                        if (designTemplate.vendorCode === _this.vendorCode) {
                            if (_this.templateId !== null) {
                                _this.allowVendorFeaturesOnly = false;
                                var vendorTemplateProperty = _this.getPropertyByName('TEMPLATE_VENDOR_CODE', _this.selectedProduct.properties);
                                if (vendorTemplateProperty !== null) {
                                    vendorTemplateProperty.value = _this.vendorCode;
                                }
                                var itTemplates = designTemplate.templates.iterator();
                                var template = null;
                                while (itTemplates.hasNext()) {
                                    ...
                                    if (template.id === _this.templateId) {
                                        deserializedHint.setChoiceIds(template.choiceIds);
                                        break;
                                    }
                                }
                            }
                            else {
                                _this.allowVendorFeaturesOnly = true;
                                var defaultVendorChoiceIds = [];
                                var featureRef = null;
                                var itFeatureRefs = designTemplate.featureRefs.iterator();
                                while (itFeatureRefs.hasNext()) {
                                    featureRef = itFeatureRefs.next();
                                    defaultVendorChoiceIds.push(featureRef.defaultChoiceId);
                                }
                                deserializedHint.setChoiceIds(_this.productTranslator.translateArrayToList(defaultVendorChoiceIds));
                            }
                        }
                    }
                }
    ...
    ProductController.prototype.getQuantitySets = function (productKey, successCallback, errorCallback) {
        var _this = this;
        var quantitySet = [];
        this.fetchProductDataById(productKey.id, productKey.version, null, function (resultProductData) {
            var property = _this.getPropertyByName('PRODUCT_QTY_SET', resultProductData.productData.product.properties);
            if (property != null) {
                var boundIt = property.getBounds().iterator();
                while (boundIt.hasNext()) {
                    var inBound = boundIt.next();
    ...
    ProductController.prototype.getFlattenedContent = function (content) {
        ...
        if (content.pageGroups) {
            var it_7 = content.pageGroups.iterator();
            while (it_7.hasNext()) {
                pageGroups.push(this.getFlattenedPageGroup(it_7.next()));
            }
    ...
    ProductController.prototype.serializeFlattenedProduct = function (flattenedProduct) {
        ...
        if (flattenedProduct.contents) {
            var it_9 = flattenedProduct.contents.iterator();
            while (it_9.hasNext()) {
                contents.push(this.getFlattenedContent(it_9.next()));
            }
            flattenedProduct.contents = contents;
        }
        if (flattenedProduct.pageExceptions) {
            var it_10 = flattenedProduct.pageExceptions.iterator();
            var _loop_6 = function () {
                var pageException = it_10.next();
                var pageRanges = [];
    ...
    ProductController.prototype.hasPrintReadyContent = function (id) {
        ...
        if (contentAssociations) {
            var it_11 = contentAssociations.iterator();
            while (it_11.hasNext()) {
                var ca = it_11.next();
    ...
    ProductController.prototype.addSizeHints = function (hints, contentRequirement, properties, isDefaultHints) {
        if ((!isDefaultHints) || (contentRequirement.resizeIfDefault)) {
            if (properties.PAGE_ORIENTATION && String(properties.PAGE_ORIENTATION).toUpperCase() === 'LANDSCAPE') {
                if (contentRequirement.bleedDimension) {
                    hints.targetWidthInInches = parseFloat(properties.MEDIA_HEIGHT) + parseFloat(contentRequirement.bleedDimension.width.start);
                    hints.targetHeightInInches = parseFloat(properties.MEDIA_WIDTH) + parseFloat(contentRequirement.bleedDimension.height.start);
                }
                else {
                ...
                if (contentRequirement.bleedDimension) {
                    hints.targetWidthInInches = parseFloat(properties.MEDIA_WIDTH) + parseFloat(contentRequirement.bleedDimension.width.start);
                    hints.targetHeightInInches = parseFloat(properties.MEDIA_HEIGHT) + parseFloat(contentRequirement.bleedDimension.height.start);
                }
                else {
    ...
    ProductController.prototype.getAllowedSizes = function () {
        var configFeature = this.getConfigFeatureChoiceProperties('MEDIA_WIDTH');
        var allowedSizes = [];
        if (configFeature) {
            ...
            while (choiceItr.hasNext()) {
                ...
                var allowedSize = {};
                allowedSize.choiceId = configChoice.id;
                while (configPropItr.hasNext()) {
                    var configProp = configPropItr.next();
                    if (configProp.name === 'MEDIA_WIDTH') {
                        allowedSize.width = configProp.value;
                    }
                    else if (configProp.name === 'MEDIA_HEIGHT') {
                        allowedSize.height = configProp.value;
                    }
                }
                allowedSizes.push(allowedSize);
            }
        }
        return allowedSizes;
    };
    ...
    ProductController.prototype.processContentPageExceptions = function (pageExceptions) {
        var _loop_7 = function (i) {
            var pe = pageExceptions[i];
            if (pe.action === 'UPDATE' || pe.action === 'DELETE') {
                var it_12 = this_4.selectedProduct.getPageExceptions().iterator();
                while (it_12.hasNext()) {
                    var pageException = it_12.next();
                    if (pe.instanceId === pageException.instanceId) {
                        this_4.selectedProduct.removePageException(pageException);
                    }
                }
            }
            if (pe.action === 'ADD' || pe.action === 'UPDATE') {
                var peInstance = null;
                var configuredIt = this_4.product.pageExceptions.iterator();
                while (configuredIt.hasNext()) {
                    var configuredPe = configuredIt.next();
                    if (Number(pe.pageExceptionId) === Number(configuredPe.id)) {
                        peInstance = this_4.productEngine.buildPageExceptionInstance(configuredPe);
                        break;
                    }
                }
                if (peInstance) {
                    peInstance.setHasContent(pe.hasContent);
                    peInstance.setInstanceId(pe.instanceId);
                    if (pe.features) {
                        for (var k = 0; k < pe.features.length; k++) {
                            var featureChoice = pe.features[k];
                            var feature = this_4.getFeatureById(featureChoice.featureId);
                            var choice = feature.choices.get(featureChoice.choiceId);
                            this_4.selectChoice(choice, feature, peInstance);
                            if (featureChoice.properties) {
                                var choiceProperties = choice.getProperties();
                                if (choiceProperties) {
                                    this_4.setPropertyValues(featureChoice.properties, peInstance);
                                }
                            }
                        }
                    }
                    if (pe.pageDetails) {
                        var virtualPageRanges_1 = [];
                        if (pe.pageDetails.contentPageRef) {
                            var tabsInsertPages = this_4.getTabsInsertRanges();
                            tabsInsertPages.sort(function (a, b) { return a - b; });
                            var _loop_8 = function (l) {
                                var pePageRange = pe.pageDetails.pageRanges[l];
                                var newSplitRanges = [];
                                newSplitRanges.push(pePageRange);
                                tabsInsertPages.forEach(function (tabInsertPage) {
                                    for (var m = 0; m < newSplitRanges.length; m++) {
                                        var newSplitRange = newSplitRanges[m];
                                        if (newSplitRange.start >= tabInsertPage) {
                                            newSplitRange.start = newSplitRange.start + 1;
                                            newSplitRange.end = newSplitRange.end + 1;
                                            break;
                                        }
                                        else if (newSplitRange.end >= tabInsertPage) {
                                            var newEnd = newSplitRange.end + 1;
                                            newSplitRange.end = tabInsertPage - 1;
                                            var newPageRange = { start: tabInsertPage + 1, end: newEnd };
                                            newSplitRanges.push(newPageRange);
                                            break;
                                        }
                                    }
                                });
                                newSplitRanges.forEach(function (newSplitRange) {
                                    virtualPageRanges_1.push(newSplitRange);
                                });
                            };
                            for (var l = 0; l < pe.pageDetails.pageRanges.length; l++) {
                                _loop_8(l);
                            }
                        }
                        else {
                            virtualPageRanges_1 = pe.pageDetails.pageRanges;
                        }
                        for (var n = 0; n < virtualPageRanges_1.length; n++) {
                            peInstance.addRange(this_4.productTranslator.translatePageRange(virtualPageRanges_1[n]));
                        }
                    }
                    this_4.selectedProduct.addPageException(peInstance);
                }
            }
        };
        var this_4 = this;
    ...
    ProductController.prototype.getTemplateVendorCodes = function () {
        var designTemplate = null;
        var vendorCodesList = [];
        if (this.productData.designTemplates) {
            var it_13 = this.productData.designTemplates.iterator();
            while (it_13.hasNext()) {
                designTemplate = it_13.next();
                vendorCodesList.push(designTemplate.vendorCode);
            }
            return vendorCodesList;
        }
        return null;
    };
    ProductController.prototype.getVendorTemplate = function (vendorCode, choiceIds) {
        var designTemplate = null;
        var template = null;
        var templateChoiceIdsArray = [];
        var bestMatch = null;
        if (this.productData.designTemplates) {
            var it_14 = this.productData.designTemplates.iterator();
            while (it_14.hasNext()) {
                designTemplate = it_14.next();
                if (designTemplate.vendorCode === vendorCode) {
                    var templatesItr = designTemplate.templates.iterator();
                    while (templatesItr.hasNext()) {
                        template = templatesItr.next();
                        templateChoiceIdsArray = template.choiceIds.toArray();
                        var matchCount = templateChoiceIdsArray.filter(function (templateChoiceId) {
                            return choiceIds.indexOf(templateChoiceId) !== -1;
                        }).length;
                        if (matchCount > 0 && (!bestMatch || matchCount > bestMatch.matchCount)) {
                            bestMatch = {
                                vendorTemplateId: template.id,
                                vendorProductId: designTemplate.vendorProductId,
                                vendorProductVersion: designTemplate.vendorProductVersion,
                                matchCount: matchCount
                            };
                        }
                        if (matchCount === choiceIds.length) {
                            return bestMatch
                                ? { vendorTemplateId: bestMatch.vendorTemplateId, vendorProductId: bestMatch.vendorProductId, vendorProductVersion: bestMatch.vendorProductVersion }
                                : { vendorTemplateId: null, vendorProductId: null, vendorProductVersion: null };
                        }
                    }
                }
            }
    ...
    ProductController.prototype.containsAnyChoice = function (feature, choiceIds) {
        var choiceIterator = feature.choices.iterator();
        while (choiceIterator.hasNext()) {
            var choice = choiceIterator.next();
