...
namespace Fedex\FedexCsp\Stdlib\Cookie;
...
class PhpCookieManager extends \Magento\Framework\Stdlib\Cookie\PhpCookieManager
{
    ...
    public function __construct(
        ...
        private CookieReaderInterface $reader,
        ...
        if (!$phpSetcookieSuccess) {
            ...
            if ($value == '') {
                $this->logger->critical(
                    __METHOD__ . ':' . __LINE__ . ' The cookie: ' . $name . ' could not be deleted.'
                );
                throw new FailureToSendException(
                    new Phrase('The cookie with "%name" cookieName couldn\'t be deleted.', $params)
                );
            } else {
                $this->logger->critical(__METHOD__ . ':' . __LINE__ . ' The cookie: ' . $name . ' could not be sent.');
                throw new FailureToSendException(
    ...
    {
        if ($name == '' || preg_match("/[=,; \t\r\n\013\014]/", $name)) {
            $this->logger->error(__METHOD__ . ':' . __LINE__ . ' Cookie is empty or contains invalid characters.');
        ...
        if ($numCookies > static::MAX_NUM_COOKIES) {
            $this->logger->critical(
                __METHOD__ . ':' . __LINE__ . ' Unable to send the cookie. Maximum number of cookies would be exceeded.'
            );
        }

        if ($sizeOfCookie > static::MAX_COOKIE_SIZE) {
            $this->logger->critical(
                __METHOD__ . ':' . __LINE__ . ' Unable to send the cookie. Size limit has been reached.'
            );
