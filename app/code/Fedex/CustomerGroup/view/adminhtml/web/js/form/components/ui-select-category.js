...
define([
    ...
    return Abstract.extend({
        defaults: {
            ...
            optgroupTmpl: 'Fedex_CustomerGroup/form/filters/elements/ui-select-optgroup-category',
        ...
        initConfig: function (config) {
            ...
            for (let i = 0; i < options.length; i++) {
                if(window.categoryB2B != 'undefined' && options[i].label === window.categoryB2B) {
                    config.options = [];
                    config.options[0] = options[i];
                    continue;  
                }
            }
        ...
        getSelected: function () {
            ...
            //localStorage.setItem("parentGroupID", selected);
