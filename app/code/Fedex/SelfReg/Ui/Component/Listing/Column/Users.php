...
class Users extends Column
{
    ...
    public function prepareDataSource(array $dataSource)
    {
        ...
        foreach ($dataSource['data']['items'] as $index => &$item) {
            if (isset($item['user_names'])) {
                $userIds = $this->parseIds($item['user_names']);
                if (!empty($userIds)) {
    ...
    private function batchFetchUserNames(array $userIds)
    {
        try {
            ...
            $this->logger->info(
                'Batch fetched user names from database',
                [
            ...
            $this->logger->error(
                'Error batch fetching user names from DB: ' . $e->getMessage(),
                [
                    'user_ids' => $userIds,
