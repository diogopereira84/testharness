...
define([
    ...
    return Column.extend({
        defaults: {
            headerTmpl: 'Fedex_SelfReg/grid/columns/multiselect',
            bodyTmpl: 'Fedex_SelfReg/grid/cells/multiselect',
        ...
        selectPage: function () {
            ...
            $(".item-count").text(selected.length);
        ...
        */
        togglePage: function () {
            if ($(".custom_header_checkbox").prop('checked') == true){
                this.selectPage();
                $(".custom-mobile-checkbox").prop('checked',true);
                $(".selected-number").show();
                $('.custom-action-multicheck-wrap').addClass('mobile-sde-check');
            } else{
                this.deselectPage();
        ...
        countSelected: function () {
            ...
            if (excluded-this.disabled().length >= 0) {
                $(".custom_header_checkbox").prop('checked',false);
                $(".custom-mobile-checkbox").prop('checked',false);
            ...
            if (selected >= 1) {
                $('.export-user-list-btn').addClass('active');
                $('.export-user-list-btn').prop('disabled', false);
                $('.export-user-list-btn').removeClass('disabled');
            } else {
            ...
            $(".item-count").text(selected);
            window.selectedids=this.selected();
        ...
        onSearch: function () {
         ...
         /**
         * Is invoked when select an checkobx using enter key
         ...
         */
         ...
         /**
          * Is invoked when select an select all checkobx using enter key
          * @param {string} element
          */
         triggerSelectAll:function(element)
         {
              $(element).trigger('click');
         },
         
         /**
          * disable the checkbox from selection if user role is admin
          * @param {string,number}
          */
          checkRole:function(role,index)
          {
            if(role=='Admin')
            {
