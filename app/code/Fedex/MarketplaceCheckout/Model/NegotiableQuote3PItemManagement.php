...
class NegotiableQuote3PItemManagement extends NegotiableQuoteItemManagement
{
    private StoreCustomerRate $storeCustomerRate;
    private int $customPricePrecision = 4;

    public function __construct(
        private CartRepositoryInterface    $quoteRepository,
        private TaxConfig                  $taxConfig,
        private NegotiableQuoteItemFactory $negotiableQuoteItemFactory,
        private ExtensionAttributesFactory $extensionFactory,
        private TotalsFactory              $quoteTotalsFactory,
        private Update                     $priceUpdater,
        private NegotiableQuoteItem        $negotiableQuoteItemResource,
        GetNegotiatedPrice         $getNegotiatedPrice,
        protected Data                       $helper,
        Customer\StoreCustomerRate $storeCustomerRate = null
    )
    {
        $this->storeCustomerRate = $storeCustomerRate ??
            ObjectManager::getInstance()->get(Customer\StoreCustomerRate::class);
        parent::__construct(
    ...
    {
        if (!$this->helper->isD224874Enable() && !$this->helper->isUploadToQuoteEnabled()) {
            return parent::updateQuoteItemsCustomPrices($quoteId, $needSave);
        }

        $quote = $this->getQuote($quoteId);
        $this->storeCustomerRate->init($quote);
        ...
        $priceLost = 0;
        $itemsToUpdate = [];
        $preserveCustomPrice = $this->preserveQuoteCustomPrice($quote, $negotiableQuote->getStatus()); //true
        ...
        foreach ($quote->getAllItems() as $key => $quoteItem) {
            ...
            if ($needRecalculateRule) {
                ...
                $storeTaxRateFactor = $this->storeCustomerRate->getStoreTaxRateFactor($quoteItem);
                $customerTaxRateFactor = $this->storeCustomerRate->getCustomerTaxRateFactor($quoteItem);
                $storeToCustomerTaxRateRatio = $storeTaxRateFactor / $customerTaxRateFactor;

                $newPrice = $price * $priceMultiplier * $storeToCustomerTaxRateRatio;
    ...
    private function resetOriginalPrice(CartInterface $quote, $needRecalculatePrice, $needRecalculateRule)
    {
        ...
        foreach ($quote->getAllItems() as $quoteItem) {
            ...
            if (!$needRecalculatePrice) {
                ...
                $taxRateFactor = $this->storeCustomerRate->getTaxRateFactor($quoteItem);
                $itemToUpdate['custom_price'] = $price * $taxRateFactor;
            }
