...
class UpdateQuoteShipping extends Action
{
    /**
    ...
    public function execute()
    {
        ...
        $locationAddress = $this->getRequest()->getPost('locationAddress');
        $locationId = $this->getRequest()->getPost('locationId');
        $quote = $this->cartFactory->create()->getQuote();
        try {
            $this->expressCheckoutHelper->setCustomerInformation($profileAddress, $quote);
            $shippingData = $this->expressCheckoutHelper
                            ->prepareShippingData($locationAddress, $locationId, $profileAddress);
            $this->expressCheckoutHelper->setShippingBillingAddress($quote, $shippingData);
            $quote->save();
            $response = ["success" => true];
        } catch (\Exception $e) {
            $this->logger->critical(__METHOD__ . ':' . __LINE__ .
            'Update shipping and billing address in quote : ' . $e->getMessage());
