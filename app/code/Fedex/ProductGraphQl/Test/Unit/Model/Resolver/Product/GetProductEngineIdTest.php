...
use Fedex\ProductGraphQl\Model\Resolver\Product\GetProductEngineId;

class GetProductEngineIdTest extends TestCase
{
    protected $fieldMock;
    protected $resolveInfoMock;
    protected $productModelMock;
    protected $getProductEngineIdMock;
    /**
    ...
    protected function setUp(): void
    {
        ...
        $this->getProductEngineIdMock = new GetProductEngineId($this->productModelMock);
    }
    ...
    public function testResolve()
    {
        ...
        $this->productModelMock->expects($this->exactly(1))->method('getProductId')->willReturn('1456773326927');
        $this->assertEquals(
            '1456773326927',
            $this->getProductEngineIdMock->resolve(
            $this->fieldMock,
            $this->contextMock,
    ...
    public function testResolveWithNoResults()
    {
        $this->modelMock->expects($this->exactly(1))->method('getId')->willReturn(542);
        $this->productModelMock->expects($this->any())->method('load')->with(542)->willReturnSelf();
        $this->productModelMock->expects($this->exactly(1))->method('getProductId')->willReturn('');
        $this->assertEquals(
            '',
            $this->getProductEngineIdMock->resolve(
                $this->fieldMock,
