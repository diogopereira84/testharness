...
use Fedex\ProductGraphQl\Model\Resolver\Product\GetIsEnableForPoc;

class GetIsEnableForPocTest extends TestCase
{
    protected $fieldMock;
    ...
    protected function setUp(): void
    {
        ...
        $this->productModelMock = $this->getMockBuilder(Product::class)
            ->disableOriginalConstructor()
            ->onlyMethods(['load'])
            ->addMethods(['getIsEnableForPoc'])
            ->getMock();

        $this->getIsEnableForPocMock = new GetIsEnableForPoc($this->productModelMock);
    }
    ...
    public function testResolve()
    {
        ...
        $this->productModelMock->expects($this->exactly(1))->method('getIsEnableForPoc')->willReturn(true);
        $this->assertTrue($this->getIsEnableForPocMock->resolve(
            $this->fieldMock,
            $this->contextMock,
    ...
    public function testResolveWithNoResults()
    {
        $this->modelMock->expects($this->exactly(1))->method('getId')->willReturn(542);
        $this->productModelMock->expects($this->any())->method('load')->with(542)->willReturnSelf();
        $this->productModelMock->expects($this->exactly(1))->method('getIsEnableForPoc')->willReturn(false);
        $this->assertFalse(
            $this->getIsEnableForPocMock->resolve(
                $this->fieldMock,
                $this->contextMock,
                $this->resolveInfoMock,
                ['model' => $this->modelMock],
