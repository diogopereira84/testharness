...
namespace Fedex\Catalog\Component\Form\Element;
...
use Magento\Ui\Component\Form\Element\Wysiwyg as WysiwygCore;
 ...
 * @api
 */
class Wysiwyg extends \Magento\PageBuilder\Component\Form\Element\Wysiwyg
{
    /**
     * @var Repository
     */
     ...
     */
    public function __construct(ContextInterface $context, FormFactory $formFactory, ConfigInterface $wysiwygConfig, CategoryAttributeRepositoryInterface $attrRepository, PageBuilderState $pageBuilderState, Config $stageConfig, array $components = [], array $data = [], array $config = [], PageBuilderConfig $pageBuilderConfig = null, bool $overrideSnapshot = false, Repository $assetRepo = null) {//NOSONAR
        ...
        if (!$pageBuilderState->isPageBuilderInUse($isEnablePageBuilder)) {
            ...
            if ($data && isset($data['config']) && isset($data['config']['wysiwygConfigData'])
                && isset($data['config']['wysiwygConfigData']['current_attribute_code'])) {
                $wysiwygConfigData['current_attribute_code'] =
                    $data['config']['wysiwygConfigData']['current_attribute_code'];
            }
            $config['wysiwygConfigData'] = $wysiwygConfigData;
        }

        WysiwygCore::__construct($context, $formFactory, $wysiwygConfig, $components, $data, $config);
    }
