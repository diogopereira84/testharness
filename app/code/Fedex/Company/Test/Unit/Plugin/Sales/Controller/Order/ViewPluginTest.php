...
use Fedex\Company\Plugin\Sales\Controller\Order\ViewPlugin;
...
class ViewPluginTest extends TestCase
{
    ...
    public function testAroundExecuteWithException()
    {
        ...
        $this->request->expects($this->exactly(2))->method('getParam')
            ->withConsecutive(['key'], ['order_id'])->willReturn($orderId);
     ...
     * @throws \Exception
     */
    public function testAroundExecuteWithCustomerGroupResult()
    {
        $orderId = 1;
        $customerId = 2;
        $result = $this->getMockBuilder(ResultInterface::class)
        ...
        $order = $this->getMockBuilder(Order::class)
            ->disableOriginalConstructor()
            ->setMethods(['getCustomerId', 'getCustomerGroupId'])
            ->getMock();
        $this->orderRepository->expects($this->once())->method('get')->with($orderId)->willReturn($order);
        $order->expects($this->once())->method('getCustomerId')->willReturn($customerId);

        $this->deliveryDataHelper->expects($this->any())->method('isCompanyAdminUser')->willReturn(true);
        $this->customerRepository->expects($this->any())->method('getById')->with($customerId)
            ->willReturn($this->customerInterfaceMock);
        $this->customerInterfaceMock->expects($this->any())->method('getGroupId')->willReturn(90);
        $order->expects($this->any())->method('getCustomerGroupId')->willReturn(90);

        $this->assertEquals($result, $this->viewPlugin->aroundExecute($this->controller, $closure));
    }

    /**
     * Test aroundExecute with user permission review order
     *
     * @return void
     */
    public function testAroundExecuteWithUserPermissionReviewOrder()
    {
        $orderId = 1;
        $customerId = 2;
        $result = $this->getMockBuilder(ResultInterface::class)
        ...
        $this->customerRepository->expects($this->any())->method('getById')->with($customerId)
            ->willReturn($this->customerInterfaceMock);
        $this->customerInterfaceMock->expects($this->any())->method('getGroupId')->willReturn(90);
        $this->reviewOrderViewModel->expects($this->once())->method('isOrderApprovalB2bEnabled')->willReturn(true);
        $this->reviewOrderViewModel->expects($this->once())->method('checkIfUserHasReviewOrderPermission')->willReturn(true);
