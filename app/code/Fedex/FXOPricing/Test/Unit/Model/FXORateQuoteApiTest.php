 ...
 *
 * @author  Yogesh Suryawanshi <yogesh.suryawanshi@infogain.com>
...
class FXORateQuoteApiTest extends TestCase
{
    /**
    ...
    protected function setUp(): void
    {
        ...
        $this->submitOrderHelperMock = $this->getMockBuilder(SubmitOrderHelper::class)
            ->setMethods([
                'getCustomerOnBehalfOf',
                'saveRetailTransactionId'
            ])->disableOriginalConstructor()
            ->getMock();
        ...
        $this->uploadToQuoteViewModel = $this->getMockBuilder(UploadToQuoteViewModel::class)
            ...
            ->setMethods([
                ...
                'updateItemsSI',
                'excludeDeletedItem',
                'updateRateRequestForFuseBiddingDiscount',
                'updateItemsForFuse',
                'updateLineItemsSkuDetails'
            ])
            ->getMock();

        $this->FXORateQuoteApi = $this->getMockBuilder(FXORateQuoteApi::class)
            ->disableOriginalConstructor()
            ->getMock();

        $this->productPriceHandler = $this->getMockBuilder(ProductPriceHandler::class)
            ->disableOriginalConstructor()
            ->getMock();

        $this->objectManager = new ObjectManager($this);
        $this->fXORateQuoteModel = $this->objectManager->getObject(
            FXORateQuoteApi::class,
            [
                'logger' => $this->logger,
                'curl' => $this->curl,
                ...
                'productPriceHandler' => $this->productPriceHandler,
                'newRelicHeaders'       => $this->newRelicHeaders
            ]
        );
        $this->reflection = new \ReflectionClass($this->fXORateQuoteModel);
        $getCurrentPageMethod = $this->reflection->getMethod('callRateQuoteApi');
     ...
     * @return array
     */
    public function quoteSetMethodsValues()
    {
        return [
            'getId',
            'getAllItems',
    ...
    {
        $reflectionClass = new \ReflectionClass(FXORateQuoteApi::class);
        $changedMethod = $reflectionClass->getMethod('getGTNNumber');
        $changedMethod->setAccessible(true);
        $this->quote->expects($this->any())->method('getData')->with('gtn')->willReturn('1234');
        $result = $changedMethod->invoke($this->fXORateQuoteModel, $this->quote);
    ...
    public function testUpdateQuoteInfo()
    {
        ...
        $changedMethod = $reflectionClass->getMethod('updateQuoteInfo');
        $changedMethod->setAccessible(true);

        $result = $changedMethod->invoke($this->fXORateQuoteModel, $this->quote, static::RATE_OUTPUT);
    ...
    {
        $reflectionClass = new \ReflectionClass(FXORateQuoteApi::class);
        $changedMethod = $reflectionClass->getMethod('updateRateForAccount');
        $changedMethod->setAccessible(true);
        ...
        $result = $changedMethod->invoke($this->fXORateQuoteModel, $this->output, $this->quote);
        $this->assertNull($result);
    }
    ...
    public function testUpdateRateForAccountWithMiraklOfferId()
    {
        ...
        $this->quote->expects($this->any())->method('getAllVisibleItems')->willReturn([0 => $this->itemMock]);
        $this->assertNull($changedMethod->invoke($this->fXORateQuoteModel, $this->output, $this->quote));
    }
    ...
    {
        $reflectionClass = new \ReflectionClass(FXORateQuoteApi::class);
        $changedMethod = $reflectionClass->getMethod('updateRateForAccountSdeDiscountFix');
        $changedMethod->setAccessible(true);
        $this->quote->expects($this->any())->method('getAllVisibleItems')->willReturn([0 => $this->itemMock]);
        $this->itemMock->expects($this->any())->method('getMiraklOfferId')->willReturn(true);
        $this->itemMock->expects($this->any())->method('getItemId')->willReturn(0);
    ...
    {
        $reflectionClass = new \ReflectionClass(FXORateQuoteApi::class);
        $changedMethod = $reflectionClass->getMethod('updateCartItems');
        $changedMethod->setAccessible(true);
        $itemsUpdatedData = [
            0 => [
                'test' => 'test'
        ...
        $result = $changedMethod->invoke($this->fXORateQuoteModel, [$this->itemMock],
            $this->output,
            $itemsUpdatedData,
            123,
            123,
    ...
    public function testupdateCartItems()
    {
        ...
        $this->itemMock->expects($this->any())->method('setIsSuperMode')->willReturnSelf();
        $this->assertNull($changedMethod->invoke($this->fXORateQuoteModel,
            [$this->itemMock],
            $this->output,
            $itemsUpdatedData,
            123,
    ...
    public function testupdateCartItemsPricesFromRAQEnabled()
    {
        ...
        $this->itemMock->expects($this->any())->method('setOriginalCustomPrice')->willReturnSelf();
        $this->itemMock->expects($this->any())->method('setIsSuperMode')->willReturnSelf();
        $this->itemMock->expects($this->once())->method('getAdditionalData')
            ->willReturn($productRetailLinePriceMock, true);
        $this->itemMock->expects($this->once())->method('setAdditionalData')
    ...
    {
        $reflectionClass = new \ReflectionClass(FXORateQuoteApi::class);
        $changedMethod = $reflectionClass->getMethod('manageOutputData');
        $changedMethod->setAccessible(true);
        $itemsUpdatedData = [
            0 => [
                'test' => 'test'
    ...
    public function testcallRateQuoteApi()
    {
        ...
        $reflectionClass = new \ReflectionClass(FXORateQuoteApi::class);
        $getCurrentPageMethod = $reflectionClass->getMethod('callRateQuoteApi');
        $getCurrentPageMethod->setAccessible(true);
        $this->assertNotNull($getCurrentPageMethod->invoke($this->fXORateQuoteModel,
            $this->quote,
            [$this->itemMock],
            $itemData,
            "UAT001",
    ...
    public function testcallRateQuoteApiWithAlert()
    {
        ...
        $this->submitOrderModelMock->expects($this->any())->method('validateRateQuoteAPIWarnings')
            ->willReturn(true);
        $this->submitOrderHelperMock->expects($this->any())->method('saveRetailTransactionId')->willReturnSelf();
        $this->quote->expects($this->any())->method('getIsAjaxRequest')->willReturn(true);
    ...
    public function testcallRateQuoteApiWithError()
    {
        ...
        $this->submitOrderModelMock->expects($this->any())->method('validateRateQuoteAPIErrors')
            ->willReturn(true);
        $this->submitOrderHelperMock->expects($this->any())->method('saveRetailTransactionId')->willReturnSelf();
        $this->quote->expects($this->any())->method('getIsAjaxRequest')->willReturn(false);
        $this->responseFactoryFactory->expects($this->any())->method('create')->willReturnSelf();
        $this->responseFactoryFactory->expects($this->any())->method('setRedirect')->willReturnSelf();
        $this->responseFactoryFactory->expects($this->any())->method('sendResponse')->willReturnSelf();
        $reflectionClass = new \ReflectionClass(FXORateQuoteApi::class);
        $getCurrentPageMethod = $reflectionClass->getMethod('callRateQuoteApi');
        $getCurrentPageMethod->setAccessible(true);
        $this->assertNull($getCurrentPageMethod->invoke($this->fXORateQuoteModel,
            $this->quote,
            [$this->itemMock],
            $itemData,
            "UAT001",
     ...
     * Test case for callRateQuoteApiWithError
     */
    public function testcallRateQuoteApiWithErrorCase2()
    {
        $itemsUpdatedData = [
            0 => [
                'test' => 'test'
        ...
        $this->fxoModel->expects($this->any())->method('saveDiscountBreakdown')->willReturnSelf();
        $this->fxoModel->expects($this->any())->method('isVolumeDiscountAppliedonItem')->willReturnSelf();
        $this->fxoModel->expects($this->any())->method('removeQuoteItem')->willReturnSelf();
        ...
        $this->urlInterfaceMock->expects($this->any())->method('getUrl')->willReturn('www.fedex.com/cart/index/index');
        $this->responseFactoryFactory->expects($this->any())->method('create')->willReturnSelf();
        $this->responseFactoryFactory->expects($this->any())->method('setRedirect')->willReturnSelf();
        $this->responseFactoryFactory->expects($this->any())->method('sendResponse')->willReturnSelf();
     ...
     * Test case for callRateQuoteApiWithError
     */
    public function testcallRateQuoteApiWithErrorCase3()
    {
        $itemsUpdatedData = [
            0 => [
                'test' => 'test'
     ...
     * Test case for callRateQuoteApiWithError
     */
    public function testcallRateQuoteApiWithErrorCase4()
    {
        $itemsUpdatedData = [
            0 => [
                'test' => 'test'
        ...
        $this->cartDataHelper->expects($this->any())->method('getRateQuoteApiUrl')
        ...
        $this->productPriceHandler->expects($this->any())->method('handle')->willReturnSelf();
        $this->uploadToQuoteViewModel->expects($this->any())->method('isUploadToQuoteEnable')
            ->willReturn(true);
        $this->uploadToQuoteViewModel->expects($this->any())->method('updateLineItemsSkuDetails')
            ->willReturnSelf();
     ...
     * Test case for getFXORateQuoteWithToggle
     */
    public function testgetFXORateQuoteWithToggleCase2()
    {
        $itemData = [
            'productAssociations' => [0 => 'test'],
            'rateApiProdRequestData' => $this->dataString
        ...
        $this->uploadToQuoteViewModel->expects($this->any())
            ->method('excludeDeletedItem')->willReturn([0 => $this->itemMock]);
        $this->uploadToQuoteViewModel->expects($this->any())
            ->method('updateRateRequestForFuseBiddingDiscount')->willReturn($this->rateQuoteDataString);
        $this->uploadToQuoteViewModel->expects($this->any())
            ->method('updateItemsForFuse')->willReturnSelf();
        $this->assertNotNull($this->fXORateQuoteModel->getFXORateQuote(
            $this->quote,
            null,
            null,
     ...
     * Test case for getFXORateQuoteWithToggle
     */
    public function testgetFXORateQuoteWithToggleCase3()
    {
        $itemsUpdatedData = [
            0 => [
                'test' => 'test'
        ...
        $this->fxoRequestBuilder->expects($this->any())->method('getShipmentId')->willReturn('12');
        $this->inStoreRecipientsBuilder->expects($this->any())->method('build')->willReturn($arrRecipientsData);
        $this->itemMock->expects($this->any())->method('getItemId')->willReturn(1);
        $this->itemMock->expects($this->any())->method('getProduct')->willReturnSelf();
        $this->quote->expects($this->any())->method('getAllVisibleItems')->willReturn([0 => $this->itemMock]);
        $this->fxoModel->expects($this->any())->method('getDbItemsCount')->willReturn(123);
        $this->fXOProductDataModel->expects($this->any())->method('iterateItems')->willreturn($itemData);
        $this->fxoRequestBuilder->expects($this->any())->method('getPickShipData')->willreturn($arrRecipientsData);
        ...
        $this->uploadToQuoteViewModel->expects($this->any())
            ->method('updateItemsForFuse')->willReturnSelf();
        $this->submitOrderHelperMock->expects($this->any())->method('getCustomerOnBehalfOf')->willReturnSelf();
        $this->assertNotNull($this->fXORateQuoteModel->getFXORateQuote(
            $this->quote,
            null,
            null,
    ...
    /**
     * Test case for testgetFXORateQuoteWithToggleCase4
     */
    public function testgetFXORateQuoteWithToggleCase4()
    {
        $itemData = [
        ];
        $authenticationDetails = [
        ...
        $this->fxoModel->expects($this->any())->method('getDbItemsCount')->willReturn(123);
        $this->fXOProductDataModel->expects($this->any())->method('iterateItems')->willreturn($itemData);
        $this->logger->expects($this->any())->method('critical')->willReturnSelf();
        $this->assertNotNull($this->fXORateQuoteModel->getFXORateQuote(
            $this->quote,
            null,
            null,
    ...
    public function testManageCartWarnings()
    {
        ...
        $this->FXORateQuoteApi = $this->getMockBuilder(FXORateQuoteApi::class)
            ...
            ->getMock();
        $reflectionClass = new \ReflectionClass(FXORateQuoteApi::class);
        $changedMethod = $reflectionClass->getMethod('manageCartWarnings');
        $changedMethod->setAccessible(true);
        $result = $changedMethod->invoke($this->FXORateQuoteApi, $rateResponse);
    ...
    public function testGetDeliveryRatePrice()
    {
        ...
        $this->assertNotNull($changedMethod->invoke($this->FXORateQuoteApi,
    ...
    {
        $reflectionClass = new \ReflectionClass(FXORateQuoteApi::class);
        $changedMethod = $reflectionClass->getMethod('calculateDeliveryLinePrice');
        $changedMethod->setAccessible(true);
        $this->assertNotNull($changedMethod->invoke($this->fXORateQuoteModel,
