...
define([
    ...
    /**
     * Variable to hold selected store marker icon
     */
    var selected_icon = null;
    /**
     * Variable to hold default store marker icon
     */
    var default_icon = null;

    /**
     * Array to store pickup location markers
     */
    var markers = [];
    ...
    var explorersRestrictedAndRecommendedProduction = false;
    var explorersD174773Fix = false;
    let isRecipientAddressEnable = false;
    var isFclCustomer = false;
    var isSelfRegCustomer = false;
    var isSdeStore = shippingFormAdditionalScript.isSdeStore();
    var hasDefaultShippingAccountNumber = false;
    let callRateApiShippingAccountValidation = false;
    let isCheckoutQuotePriceDashable = false;
    let isCustomerShippingAccount3PEnabled = false;
    ...
    var shippingLocationCancelButton = 'shipping-location-cancel';
    var shippingLocationSaveButton = 'shipping-location-continue';
    ...
    let isD180202ToggleEnable = false;
    let isCommercial = false;
    let isPersonalAddressBook = false;
    var explorersProductionLocationFix = false;
    let techTitansProductionLocationFix = false;
    let isEproUploadtoQuoteToggle = false;
    var isExplorersAddressClassificationFixToggleEnable = false;
    var isD193257ToggleEnable = false;
    var maegeeks_pobox_validation = false;
    let isFuseBidding = false;
    var tiger_d203990 = false;
    var explorers_e450676_personal_address_book = false;
    var tiger_d213977 = false;
    var mazegeeksAllowCustomerChooseProductionLocationUpdates = false;
    let isProductionLocationAutomaticallySelected = false;
    if (isCheckoutConfigAvaliable) {
        /**
         * Checks if current user is FCL or not
         */
        isLoggedIn = typeof (window.checkoutConfig.is_logged_in) != 'undefined' && window.checkoutConfig.is_logged_in != null ? window.checkoutConfig.is_logged_in : false;

        explorersRestrictedAndRecommendedProduction = typeof (window.checkoutConfig.explorers_restricted_and_recommended_production) != 'undefined' && window.checkoutConfig.explorers_restricted_and_recommended_production != null ? window.checkoutConfig.explorers_restricted_and_recommended_production : false;
        mazegeeksAllowCustomerChooseProductionLocationUpdates = typeof (window.checkoutConfig.mazegeeks_e_482379_allow_customer_to_choose_production_location_updates) != 'undefined' && window.checkoutConfig.mazegeeks_e_482379_allow_customer_to_choose_production_location_updates != null ? window.checkoutConfig.mazegeeks_e_482379_allow_customer_to_choose_production_location_updates : false;

        if (explorersRestrictedAndRecommendedProduction || mazegeeksAllowCustomerChooseProductionLocationUpdates) {
        ...
        /**
         *  Call Rate API for Shipping Account Validation
         */
        callRateApiShippingAccountValidation = typeof (window.checkoutConfig.armada_call_rate_api_shipping_validation) != 'undefined' && window.checkoutConfig.armada_call_rate_api_shipping_validation != null ? window.checkoutConfig.armada_call_rate_api_shipping_validation : false;

        isCheckoutQuotePriceDashable = typeof (window.checkoutConfig.is_quote_price_is_dashable) != 'undefined' && window.checkoutConfig.is_quote_price_is_dashable != null ? window.checkoutConfig.is_quote_price_is_dashable : false;
        isCustomerShippingAccount3PEnabled = window.checkoutConfig.isCustomerShippingAccount3PEnabled;

        isEproUploadtoQuoteToggle = typeof window.checkoutConfig.explorers_epro_upload_to_quote != 'undefined' ? window.checkoutConfig.explorers_epro_upload_to_quote : false;

        isRecipientAddressEnable = typeof window.checkoutConfig.is_recipient_address_from_po != 'undefined' ? window.checkoutConfig.is_recipient_address_from_po : false;
        isD180202ToggleEnable = window?.checkoutConfig?.isD180202ToggleEnable;
        isCommercial = window?.checkoutConfig?.is_commercial;
        isPersonalAddressBook = window?.checkoutConfig?.is_personal_address_book;
        ...
        explorersProductionLocationFix = typeof (window.checkoutConfig.explorers_d188299_production_location_fix) != 'undefined' && window.checkoutConfig.explorers_d188299_production_location_fix != null ? window.checkoutConfig.explorers_d188299_production_location_fix : false;
        ...
        maegeeks_pobox_validation = typeof window.checkoutConfig.maegeeks_pobox_validation != 'undefined' ? window.checkoutConfig.maegeeks_pobox_validation : false;
        ...
        tiger_d203990 = typeof window.checkoutConfig.tiger_d203990 != 'undefined' ? window.checkoutConfig.tiger_d203990 : false;
        ...
        tiger_d213977 = typeof window.checkoutConfig.tiger_d213977 != 'undefined' ? window.checkoutConfig.tiger_d213977 : false;
    }

    // fix the order success page image icon breaking issue
    if (window.e383157Toggle) {
        fxoStorage.set("infoUrl", window.checkoutConfig.media_url + "/information.png");
        fxoStorage.set("product_image_data", window.checkoutConfig.product_image_data);
    } else {
        localStorage.setItem("infoUrl", window.checkoutConfig.media_url + "/information.png");
        localStorage.setItem("product_image_data", JSON.stringify(window.checkoutConfig.product_image_data));
    }
    ...
    if (isOutSourced && !isLoggedIns || !window.checkoutConfig.both) { //D-84529
    ...
    if (!window.checkoutConfig.both) {
        var pickShowVal = window.checkoutConfig.is_pickup;
        var shipShowVal = window.checkoutConfig.is_delivery;
        var switchLabel = false;
    }

    let notDefined, isShipping, isPickUp;
    if (window.e383157Toggle) {
        notDefined = (undefined == fxoStorage.get("pickupkey")) && (undefined == fxoStorage.get("shipkey"));
        isShipping = ((fxoStorage.get("shipkey") == true) && (fxoStorage.get("pickupkey") == false)) || ((fxoStorage.get("shipkey") == "true") && (fxoStorage.get("pickupkey") == "false"));
        isPickUp = ((fxoStorage.get("shipkey") == false) && (fxoStorage.get("pickupkey") == true)) || ((fxoStorage.get("shipkey") == "false") && (fxoStorage.get("pickupkey") == "true"));
    } else {
        notDefined = (undefined == localStorage.getItem("pickupkey")) && (undefined == localStorage.getItem("shipkey"));
    ...
    if (window.checkoutConfig.both && isPickUp) {
        var pickShowVal = true;
        var shipShowVal = false;
        var switchLabel = true;
        $(".opc-block-shipping-information .alertbox").hide();
    }
    if (window.checkoutConfig.both && isShipping) {
        var pickShowVal = false;
        var shipShowVal = true;
        var switchLabel = true;
        $(".opc-block-shipping-information .alertbox").hide();
    }
    if (!isLoggedIn) {
    ...
    return Component.extend({
        ...
        locationIcon: ko.observable(window.checkoutConfig.media_url + "/location.jpg"),
        hcoinfoIcon: ko.observable(window.checkoutConfig.media_url + "/hco-info-image.png"),
        alertIcon: ko.observable(window.checkoutConfig.media_url + "/Alert.png"),
        limitedTimeIconUrl: require.toUrl('images/limited_time.png'),
        customBillingShipping: ko.observableArray(window.checkoutConfig.custom_billing_shipping),
        isExpectedDeliveryDateEnabled: ko.observable(window?.checkoutConfig?.isExpectedDeliveryDateEnabled),
        isPriorityPrintLimitedTimeToggle: ko.observable(window?.checkoutConfig?.sgc_priority_print_limited_time_tag),
        isPromiseTimePickupOptionsToggle: ko.observable(window?.checkoutConfig?.sgc_promise_time_pickup_options),
        isD236651Enabled: ko.observable(window?.checkoutConfig?.sgc_D_236651),
         ...
         */
        calenderIcon: ko.observable(window.checkoutConfig.media_url + "/calender.png"),
        /**
        ...
        initialize: function () {
            ...
            self.visible.subscribe(function (visibleFlag) {
                if (visibleFlag && window.FDXPAGEID) {
                    var gdlPageId = window.FDXPAGEID + '/shipping';
            ...
            self.chosenDeliveryMethod.subscribe((newMethod) => {
                if (['shipping', 'pick-up'].includes(newMethod)) {
                    ...
                    if (window.checkoutConfig.is_sde_store && !chosenDeliveryMethod) {
                        if (window.checkoutConfig.is_pickup) {
                            newMethod = 'pick-up';
                        } else {
                            newMethod = 'shipping';
                        }
            ...
            let profileSession = window.checkoutConfig.retail_profile_session;
            ...
            self.rates.subscribe((newRates) => {
                if (newRates.length) {
                    ...
                    if (isD180202ToggleEnable && isCommercial) {
            ...
            let isSde = window.checkoutConfig.is_sde != undefined ? window.checkoutConfig.is_sde : false;
            self.isSde(isSde);
            ...
            /**
             * Clicking action URL MObile Checkout view
             */
            ...
            $(document).on('keyup focus', 'div[name="shippingAddress.custom_attributes.ext"] .input-text', function (e) {
            ...
            $(document).on('click', '.place-pickup-order', function (e) {
                $('.error-container').addClass('api-error-hide');
                $('.change-deliv-method-msg-outer-most').hide();
            });

            $(document).on('click', '#checkout-ship-button', function (e) {
                $('.error-container').addClass('api-error-hide');
                $('.change-deliv-method-msg-outer-most').hide();
            });

            $(document).on('click', '#place-pickup-order-mobile', function (e) {
            ...
            /** B-878274 | show recommended location in shipment flow **/
            $(document).ready(function () {

                var restrictedStoreData = window.checkoutConfig.restricted_production_location;
            ...
            $(document).on('change', ".custom_row_radio", function (e) {
                ...
                let city = null,
                    ...
                    lastName = null,
                    phoneNumber = null,
                    phoneNumberExten = null,
                    street1 = '';
   
                let contactId = $(this).val();
                let idSelector = "#idscheck" + contactId;
                var dataRow = $(idSelector).closest('tr');
                let name = dataRow.find('td[data-th="FIRST NAME"] .data-grid-cell-content').text();
                ...
                let addressPart2 = addressTwo ? addressTwo.split(',') : [];
                let addressPart1 = addressOne ? addressOne.split(',') : [];
                street1 = addressPart1[0] ? addressPart1[0].trim() : null;
                city = addressPart1[1] ? addressPart1[1].trim() : null;
                let stateZipcode = addressPart2.slice(1).join(', ').trim();
                let stateZipcodeArray = stateZipcode ? stateZipcode.split(' ') : [];
                stateCode = stateZipcodeArray.length > 0 && stateZipcodeArray[0] ? stateZipcodeArray[0].trim() : null;
                pinCode = stateZipcodeArray.length > 1 && stateZipcodeArray[1] ? stateZipcodeArray[1].trim() : null;  
                let phoneInput = $('#phoneNumber_' + contactId);
                phoneNumber = phoneInput.length && phoneInput.val().trim() !== '' ? phoneInput.val().trim() : null;
                let phoneExtInput = $('#phoneNumberExten_' + contactId);
                phoneNumberExten = phoneExtInput.length && phoneExtInput.val().trim() !== '' ? phoneExtInput.val().trim() : null;
                $(".form-shipping-address input[name='custom_attributes[ext]']").val(phoneNumberExten);
                addAddressbookData =
                {
                    firstName: firstName,
                    lastName: lastName,
                    ...
                    phoneNumberExten: phoneNumberExten,
                    city: city,
                    pinCode: pinCode,
                    stateCode: stateCode
                };
            ...
            $(document).on('click', '#change-p-location', function (e) {
                ...
                var popup = modal(options, $('.delivery-locations-info'));
                $(".delivery-locations-info").modal("openModal");
                //self.showRecommendedLocationForShipment();
            });

            $(document).on('keyup onkeypress', 'input[name="popup-zipcode"]', function (event) {
            ...
            $(document).on('keyup blur', 'input[name=alternate_firstname]', function (e) {
                let [isValid, errorMessage] = self.validateInputNameField(e.target, e);
            ...
            $(document).on("click", "#get-Shipping-result", async function (e) {
                ...
                if (!isRecipientAddressEnable && typeof event !== 'undefined') {
            ...
            $(document).on('change input', '.fedex_account_number-field', function (e) {
                const shippingAccountValue = e.target.value;

                if (shippingAccountValue.length) {
            ...
            $(document).on("keyup", '.fedex_account_number-field', function (e) {
                ...
                let shippingAccountNumber = $(".fedex_account_number-field").val();
            ...
            $(document).ajaxComplete(function () {
                if (isFclCustomer == true || isSelfRegCustomer) {
                    self.setCustomerShippingAddess();
            ...
            */
            ...
            if (
                window.checkoutConfig.both && isPickUp
            ) {
            ...
            if (!isCustomerShippingAccount3PEnabled && this.isFullMarketplaceQuote()) {
        ...
        renderCustomShippingFields: function () {
            ...
            if ((isSdeStore || isSelfRegCustomer || isEpro) && window.checkoutConfig.custom_billing_shipping?.length > 0) {
                const shippingArray = window.checkoutConfig.custom_billing_shipping[0];
        ...
        checkCustomShippingEditable: function () {
            ...
            if (shippingAccEditable === '0') {
                removeAccBtn.hide();
                if (isSelfRegCustomer && shippingAccInput.hasClass("ship-not-prepopulated")) {
                    removeAccBtn.show();
                } else if (isSdeStore && this.isAutopopulate()) {
                    removeAccBtn.show();
                }
            }
            else if (shippingAccEditable === '1') {
                removeAccBtn.show();
            }
        },
        ...
        isProductionLocationOn: function () {
            return window.checkoutConfig.is_production_location;
        },
        ...
        checkIsPeakSeasonMessageShowing: function () {
            return window.checkoutConfig.is_covidPeakSeason;
        },
                    ...
                    success: function (data) {
                        ...
                        pickUpResponceDataPaged.forEach(function (element) {
                            if (window.tiger_d195327_toggle) {
                                var isPremium = element.location.premium;
                                if (isPremium === true) {
                                    arr[i++] = element.location.id;
                                }
                            } else {
                                var hco = element.location.type;
                                if (hco == 'HOTEL_CONVENTION') {
                                    arr[i++] = element.location.id;
                                }
                            }
                        });
                        ...
                        if (!window.checkoutConfig.is_production_location && !explorersRestrictedAndRecommendedProduction) {
                            $(".recommended-store-label-pickup").hide();
                        }
                        ...
                        if (pickupRequestExecution) {
                            $(".pick-up-button").on('change', function () {
                                ...
                                if (
                                    $this.closest(".box-container").find('.pickupPageLocation').length &&
                                 ...
                                 * B-1242868
                                 */
                                if ((isFclCustomer || window.checkoutConfig != undefined) && window.checkoutConfig.fcl_login_customer_detail.first_name != undefined) {
                                    if (!($(".pickup-location-list-container .contact-from-container .contact-first-name .contact-fname").val())) {
                                        var fclFirstName = window.checkoutConfig.fcl_login_customer_detail.first_name;
                                    ...
                                    if (!($(".pickup-location-list-container .contact-from-container .contact-last-name .contact-lname").val())) {
                                        var fclLastName = window.checkoutConfig.fcl_login_customer_detail.last_name;
                                    ...
                                    if (!($(".pickup-location-list-container .contact-from-container .contact-phone-no .contact-number").val())) {
                                        var fclContactNumber = window.checkoutConfig.fcl_login_customer_detail.contact_number;
                                    ...
                                    if (!($(".pickup-location-list-container .contact-from-container .contact-phone-ext .contact-ext").val())) {
                                        var fclExtNumber = window.checkoutConfig.fcl_login_customer_detail.contact_ext;
                                        $(".pickup-location-list-container .contact-from-container .contact-phone-ext .contact-ext").val(window.checkoutConfig.fcl_login_customer_detail.contact_ext);
                                    }

                                    if (!($(".pickup-location-list-container .contact-from-container .contact-email-container .contact-email").val())) {
                                        var fclEmailAddress = window.checkoutConfig.fcl_login_customer_detail.email_address;
                                ...
                                $.ajax({
                                    ...
                                    if (typeof (response) != "undefined" && typeof (response.rateDetails) != "undefined") {
                                        response.rateDetails.forEach((rateDetail) => {
                                            ...
                                            if (typeof rateDetail.discounts != "undefined") {
                                                discountResult = rateResponseHandler.getTotalDiscountAmount(rateDetail, totalDiscountAmount, promoDiscountAmount, accountDiscountAmount, volumeDiscountAmount, discountResult);
                                    ...
                                    if (accountDiscountAmount || volumeDiscountAmount || promoDiscountAmount || shippingDiscountAmount) {
                                        $(".discount_breakdown tbody tr.discount").remove();
                                    }
                                    if (accountDiscountAmount == 0 && volumeDiscountAmount == 0 && promoDiscountAmount == 0 && shippingDiscountAmount == 0) {
                        ...
                        if (validattionContactForm) {
                            ...
                            $(".contact-number").on("keyup", function () {
                            ...
                            $(".alternate-number").on("keyup", function () {
        ...
        /**
         * to check if B2b pending order approval is enable
         */
        isB2bOrderApproval: function () {
            ...
            let isB2bOrderApproaval = false;

            if (isCheckoutConfigAvaliable) {
                isB2bOrderApproaval = typeof (window.checkoutConfig.xmen_order_approval_b2b_enabled) != 'undefined' && window.checkoutConfig.xmen_order_approval_b2b_enabled != null ? window.checkoutConfig.xmen_order_approval_b2b_enabled : false;
            }
            if (isB2bOrderApproaval) {
        ...
        initPickUpModal: function () {
            let self = this;
            pickupTimeModal = $('#pickuptimeModal');
            modalOptions = {
                type: 'popup',
            ...
            pickupTimeModal.modal(modalOptions);

            pickupTimeModal.on('modalopened', function () {

                //Setting to next 30days
                let now = new Date();
                now.setDate(now.getDate() + 29);

                let earliestpickupDate = self.koPickupDateHidden();
                ...
                let earliest = self.isPromiseTimePickupOptionsToggle() && self.koSelectedPickupLaterDate() ? self.koSelectedPickupLaterDate() : new Date(earliestPickupDay);
                ...
                let closedDays = [];
                for (let i = 0; i < self.koPickupTimeAvailability().length; i++) {
                    let counter = self.koPickupTimeAvailability()[i],
                ...
                pickupDatePicker.datepicker("option", "beforeShowDay", function (date) { return noActiveDay(date, closedDays); });
                if (self.isPromiseTimePickupOptionsToggle()) {
                    pickupDatePicker.datepicker("option", "minDate", startDate);
                } else {
                    pickupDatePicker.datepicker("option", "minDate", earliest);
                }
                pickupDatePicker.datepicker("option", "maxDate", now);
                pickupDatePicker.datepicker("setDate", earliest);
            ...
            pickupTimeModal.on('modalclosed', function () {
                if (self.isPromiseTimePickupOptionsToggle() && !self.koSelectedPickupLaterDate()) {
                    let defaultDate = self.koPickupDate();
            ...
            pickupDatePicker = $('#prefdatetimepicker');
            pickupDatePicker.datepicker({
                hideIfNoPrevNext: true,
                dateFormat: 'yy/mm/dd',
                onSelect: function (selectedDate) {
                    return selectedTimeSlotDropdown(selectedDate);
                }
            });

            function selectedTimeSlotDropdown(selectedDate) {
                self.koSelectedDate(selectedDate);
                let selectedDateFormat = new Date(selectedDate);
                let selectedDateCompare = self.removeTimeFromDateFormat(selectedDateFormat);
                let selectDay = selectedDateFormat.getDay();
                let defaultDateFormat = new Date();
                let defaultDateCompare = self.removeTimeFromDateFormat(defaultDateFormat);

                let checkFlag = (selectedDateCompare.getTime() === defaultDateCompare.getTime()) ? true : false;
                let isFutureDate = (selectedDateCompare.getTime() === self.koEarliestDayFormat()) ? true : false;

                //To find Selected Time Range
                self.getSelectedTimeRange(selectDay, checkFlag, isFutureDate);
        ...
        showPickupTimeModal: function () {
            ...
            pickupTimeModal.modal(modalOptions).modal("openModal");
        },
        ...
        getSelectedTimeRange: function (day, flagToday, flagFutureEarliest) {
            let self = this;
            let timeRanges;
            self.koFlagIsToday(flagToday);
            self.koFlagIsFutureEarlist(flagFutureEarliest);
            switch (day) {
                case 0:
                    timeRanges = self.koPickupTimeAvailability().find(element => element.day === 'Sun');
                    break;
                case 1:
                    timeRanges = self.koPickupTimeAvailability().find(element => element.day === 'Mon');
                    break;
                case 2:
                    timeRanges = self.koPickupTimeAvailability().find(element => element.day === 'Tue');
                    break;
                case 3:
                    timeRanges = self.koPickupTimeAvailability().find(element => element.day === 'Wed');
                    break;
                case 4:
                    timeRanges = self.koPickupTimeAvailability().find(element => element.day === 'Thu');
                    break;
                case 5:
                    timeRanges = self.koPickupTimeAvailability().find(element => element.day === 'Fri');
                    break;
                case 6:
                    timeRanges = self.koPickupTimeAvailability().find(element => element.day === 'Sat');
            }

            self.koSelectedTimeRange(timeRanges.range);
        },
            ...
            if (openTime.indexOf('A.M') > -1 && closeTime.indexOf('A.M') == -1) {
                if (initOpenTime < twelveHour) {
                    for (let k = initOpenTime; k < twelveHour; k++) {
                        ...
                        let checkHours = self.convertToTwentyFourHours(timeFormat);
                    ...
                    if (initCloseTime < twelveHour) {
                        let checkHours = self.convertToTwentyFourHours(twelvePM);
                        if ((flag && checkHours <= timenow) || (flag && checkHours < earliestHour)) {
                ...
                if (initOpenTime < twelveHour) {
                    ...
                    if (initCloseTime < twelveHour) {
                        let checkHours = self.convertToTwentyFourHours(twelveAM);
                        if ((flag && checkHours <= timenow) || (flag && checkHours < earliestHour)) {
        ...
        // happens when pickup location is SELECTED;
        getEarliestPickupDateTime: function (dateTimeEarliest) {
            //B-1516871: Pickup date formating.
            const regex = /^(.*\d{1,2}(th|st|nd|rd))\s+(.*)$/;
            const match = dateTimeEarliest.match(regex);
            let earliestPickupArray = [
                match[1].trim(), match[3].trim()
            ]

            let earliestPickupDate = earliestPickupArray[0];
            earliestPickupDate = earliestPickupDate.split(',').join(', ');
            let earliestPickupTime = earliestPickupArray[1];
            let earliestPickupDateTimeHtml = earliestPickupDate + '</br>' + earliestPickupTime;
            return earliestPickupDateTimeHtml;
        },
        ...
        convertToTwentyFourHours: function (timeToConvert) {
        ...
        updatePickupDateTimeFormat: function () {
            ...
            let optDate = pickupDatePicker.datepicker("getDate");
            optDate = new Date(optDate);
            self.koSelectedDate(optDate);// resetting selected Date
            if (self.isPromiseTimePickupOptionsToggle()) {
                self.koSelectedPickupLaterDate(optDate);
            }
            let optTime = self.selectedTimeslot();

            if (typeof (optTime) !== "undefined") {
                let hours = self.convertToTwentyFourHours(optTime);
                ...
                self.callRateQuoteApi();
                if (window.e383157Toggle) {
                    fxoStorage.set("pickupDateTime", pickupDateTimeDisplay);
                } else {
                    localStorage.setItem("pickupDateTime", pickupDateTimeDisplay);
                }
                self.koEarliestPickupDateTime(pickupDateTimeDisplay);
                if (self.isPromiseTimePickupOptionsToggle()) {
                    self.koFormattedPickupLaterDate(pickupLaterDateTimeDisplay);
                    if (self.currentPickupLaterRadio()) {
                        self.currentPickupLaterRadio().siblings('#pickup_later_date').show();
                    }
                }
        ...
        callRateQuoteApi: async function () {
            let requestUrl = urlBuilder.build("delivery/index/ratequoteapi");
            let locationId;
            if (window.e383157Toggle) {
        ...
        updatePickupTime: function () {
            ...
            let optDate = pickupDatePicker.datepicker("getDate");
            optDate = new Date(optDate);
            self.koSelectedDate(null);// resetting selected Date
            let optTime = self.selectedTimeslot();
            ...
            if (typeof (optTime) !== "undefined") {
                ...
                self.koEarliestPickupDateTime(earliestPickupDateTimeLabel);
            }
        },
        getPickupLocationAndAddress: function () {
            let self = this;
            let prefPickupName = self.koPickupName(),
                prefPickupAddress = self.koPickupAddress();

            let prefPickupLocationAddressTxt = prefPickupName + ' located at ' + prefPickupAddress;

            return prefPickupLocationAddressTxt;
        },

        getPickupDateTime: function () {
            let self = this;
            let prefPickupDate = self.koPickupDate();
            let prefPickupDateTimeHtml = '<b>Earliest pickup time: </b>' + prefPickupDate;

            return prefPickupDateTimeHtml;
        },
        ...
        savePickupInformation: function (triggeredByButton = false) {
            ...
            let information, isPickup, expressCheckout, pickupData;
            if (window.e383157Toggle) {
            ...
            if (
                ...
                var pickup_location_name = "";
                var pickup_location_street = "";
                var pickup_location_city = "";
                var pickup_location_state = "";
                var pickup_location_zipcode = "";
                var pickup_location_country = "";
                var pickup_location_date = "";
                var contact_fname = $('.contact-fname').val();
                var contact_lname = $('.contact-lname').val();
                var contact_email = $('.contact-email').val();
                /* D-87462 */
                var contact_ext = $('.contact-ext').val();
                //B-1126844 | update cart items price
                var rateApiResponse = $('#rateApiResponse').val();

                var contact_number_pickup = null;
                var contact_number_pickup = $('.contact-number').val().replace(" ", "").replace("(", "").replace(")", "").replace("-", '') + ' ' + $('.contact-ext').val();
                var contact_number = $('.contact-number').val().replace(" ", "").replace("(", "").replace(")", "").replace("-", '');
                var alternate_fname = $('.alternate-fname').val();
                var alternate_lname = $('.alternate-lname').val();
                var alternate_email = $('.alternate-email').val();
                /* D-87462 */
                var alternate_ext = $('#alternate-ext').val();
                var alternate_number = $('.alternate-number').val().replace(" ", "").replace("(", "").replace(")", "").replace("-", '');
                var isAlternatePerson = $('.alternate-from-container').css('display') == 'none' ? false : true;
                ...
                $(".pickup-address").each(
                    function () {
                        ...
                        if ($(this).hasClass("selected-item")) {
                            pickup_location_name = $(this).children(".pickup-location-name").text();
                            pickup_location_street = $(this).children(".pickup-location-street").text();
                            pickup_location_city = $(this).children(".pickup-location-city").text();
                            pickup_location_date = (updatedChangedPickupDateTime == null) ? $(this).children(".pickup-location-estimate-date").text() : updatedChangedPickupDateTime;
                            pickup_location_state = $(this).children(".pickup-location-state").text();
                            pickup_location_zipcode = $(this).children(".pickup-location-zipcode").text();
                            pickup_location_country = $(this).find(".pickup-location-country").text();
                            pickup_location_id = $(this).find(".pickup-location-id").text();
                        }
                    }
                );
                ...
                information = {
                    ...
                    addressInformation: {
                        ...
                        shipping_detail: {
                            ...
                            method_code: "PICKUP",
                            carrier_title: "Fedex Store Pickup",
                            method_title: pickup_location_id,
                            amount: 0,
                            base_amount: 0,
                            available: true,
                            error_message: "",
                    ...
                    rateapi_response: rateApiResponse
                };
        ...
        selectShippingMethod: function (shippingMethod, event) {
            ...
            var radioBtnId = checkoutAdditionalScript.checkShippingOptionId(event);
            ...
            if (regionCode != 'undefined' && regionCode != '') {
                ...
                const existingMethodIndex =
                    selectedShippingMethodsStorage
                        .findIndex((method) => {
                            if (
                                newShippingData.marketplace
                                && method.carrier_title === newShippingData.carrier_title
                            ) {
                                return method;
                            }
                            ...
                            if (
                                !newShippingData.marketplace
                                && method.carrier_code === newShippingData.carrier_code
                            ) {
                                return method;
                            }
                        });
                ...
                $.ajax({
                    ...
                    totalNetAmount = priceFormatWithCurrency(totalNetAmount);
                    var cartPrice = priceFormatWithCurrency(calculate("taxableAmount"));
                    grossAmount = priceFormatWithCurrency(grossAmount);
                    ...
                    if (!isCustomerShippingAccount3PEnabled) {
        ...
        setShippingInformation: function (gotToNextStep = true) {
            ...
            if (this.validateShippingInformation()) {
                ...
                quote.shippingMethod()['fedexShipAccountNumber'] = shipAccountNumber;
        ...
        setDefaultShipping: function () {
            let fcl_shipping_data_status = (typeof window.checkoutConfig.fcl_customer_default_shipping_data.status !== "undefined") ? window.checkoutConfig.fcl_customer_default_shipping_data.status : 'Failure';
            if (isSelfRegCustomer && window.checkoutConfig.fcl_customer_default_shipping_data !== undefined && window.checkoutConfig.fcl_customer_default_shipping_data != "" && fcl_shipping_data_status != "Failure") {
                var customerInfo = customerData.get('checkout-data')();
        ...
        showPopupDeliveryMethodChange: function () {
            ...
            if (deliveryMethod && (typeof deliveryMethod !== 'undefined')) {
                const messageTitle = window.checkoutConfig.toastTitle;
                const pickupMessage = window.checkoutConfig.toastPickupContent;
                const deliveryMessage = window.checkoutConfig.toastShippingContent;
        ...
        validateInputNameField: function (element, e = null) {
            ...
            if (isD193257ToggleEnable && (nameRegex.test(elementValue))) {
                isValid = false;
        ...
        validateContactFirstName: function (e = null) {
            return this.validateContactOrAlternateInputNameField('.contact-fname', 'contact-error', e);
        },

        validateContactLastName: function (e = null) {
            return this.validateContactOrAlternateInputNameField('.contact-lname', 'contact-error', e);
        },

        validateContactNumber: function () {
            let contactNumber = $('.contact-number').val(),
                contactNumberClean = $('.contact-number').val().replace(/\D/g, '');
            if (contactNumberClean.length === 0) {
                $('.contact-number').addClass('contact-error');
                $('.contact-number-error').show();
                $('.contact-number-error').html("Phone Number is required");
                self.formError = false;
                return false;
            } else if (contactNumberClean.length < 10) {
                $('.contact-number').addClass('contact-error');
                $('.contact-number-error').show();
                $('.contact-number-error').html("Please enter a valid phone number");
                self.formError = false;
                return false;
            } else if (!telephoneRegex.test(contactNumber)) {
                $('.contact-number').addClass('alternate-error');
                $('.contact-number-error').show();
                $('.contact-number-error').html("Please enter a valid phone number");
                self.formError = false;
                return false;
            } else {
                $('.contact-number').removeClass('contact-error');
        ...
        validateContactEmail: function () {
            ...
            if (userEmailValue.length == 0) {
                ...
                $('.contact-email-error').html("Email Address is required");
                self.formError = false;
                return false;
            } else if (!regex.test(userEmailValue) && userEmailValue.length > 0) {
                $('.contact-email').addClass('contact-error');
                $('.contact-email-error').show();
                $('.contact-email-error').html("Please enter a valid email address");
                self.formError = false;
                return false;
            } else if (userEmailValue.length > 150) {
                $('.contact-email').addClass('contact-error');
                $('.contact-email-error').show();
                $('.contact-email-error').html("Email address should not be greater than 150 characters");
                ...
                $('.contact-email').removeClass('contact-error');
        ...
        validateAlternateFirstName: function (e = null) {
            return this.validateContactOrAlternateInputNameField('.alternate-fname', 'alternate-error', e);
        },

        validateAlternateLastName: function (e = null) {
            return this.validateContactOrAlternateInputNameField('.alternate-lname', 'alternate-error', e);
        },

        validateAlternateNumber: function () {
            let alternateContactNumber = $('.alternate-number').val(),
                alternateContactNumberClean = $('.alternate-number').val().replace(/\D/g, '');
            if (alternateContactNumberClean.length === 0) {
                $('.alternate-number').addClass('alternate-error');
                $('.alternate-number-error').show();
                $('.alternate-number-error').html("Phone Number is required");
                self.formError = false;
                return false;
            } else if (alternateContactNumberClean.length < 10) {
                $('.alternate-number').addClass('alternate-error');
                $('.alternate-number-error').show();
                $('.alternate-number-error').html("Please enter a valid phone number");
                self.formError = false;
                return false;
                ...
                $('.alternate-number').removeClass('alternate-error');
                $('.alternate-number-error').hide();
                self.formError = false;
                return true;
            }
        },
        ...
        validateAlternateEmail: function () {
            ...
            let regex = /^([_\-'.0-9a-zA-Z]+)@([_\-'.0-9a-zA-Z]+)\.([a-zA-Z]{2,7})$/;
            if (userEmailValue.length == 0) {
                $('.alternate-email').addClass('alternate-error');
                $('.alternate-email-error').show();
                $('.alternate-email-error').html("Email Address is required");
                self.formError = false;
                return false;
            } else if (!regex.test(userEmailValue) && userEmailValue.length > 0) {
                $('.alternate-email').addClass('alternate-error');
                $('.alternate-email-error').show();
                $('.alternate-email-error').html("Please enter a valid email address");
                self.formError = false;
                return false;
            } else if (userEmailValue.length > 150) {
                $('.alternate-email').addClass('alternate-error');
                $('.alternate-email-error').show();
                $('.alternate-email-error').html("Email address should not be greater than 150 characters");
                $(".place-pickup-order").attr('disabled', 'disabled');
                $(".place-pickup-order").addClass("place-pickup-order-disabled");
                expressCheckoutShiptoBuilder.disabledReviewButtonForPickup();
                self.formError = false;
                ...
                $('.alternate-email').removeClass('alternate-error');
                $('.alternate-email-error').hide();
                self.formError = false;
                return true;
            }
        },
        ...
        validateContactForm: function () {
            ...
            let nameFieldValidation = true;
            if (this.isCheckoutQuotePriceDashable()) {
                if ($(".upload-to-quote #zipcodeLocation").val().length < 5 && $(".upload-to-quote #quote-pickup-search-form-container").css("display") != 'none') {
                    zipCodeFieldValidation = false;
                }
            }
            //First and Last Name special character validation D-193257
            if (isD193257ToggleEnable &&
                (nameRegex.test($(".contact-fname").val().trim())) ||
                (nameRegex.test($(".contact-lname").val().trim()))
            ) {
                nameFieldValidation = false;
            }
            if ($(".alternate-from-container").css('display') == 'none') {
                if (window.checkoutConfig.is_epro && window.checkoutConfig.explorers_site_level_quoting_stores && window.checkoutConfig.is_quote_price_is_dashable) {
                    let zipCodeLocationLength = $(".upload-to-quote #zipcodeLocation").val().length;
                    if (zipCodeLocationLength >= 5 && ($(".contact-fname").val().trim().length > 1 && $(".contact-fname").val().trim().length <= 30) &&
                        ($(".contact-lname").val().trim().length > 1 && $(".contact-lname").val().trim().length <= 30) &&
                        telephoneRegex.test($(".contact-number").val()) &&
                        regex.test($(".contact-email").val()) && nameFieldValidation) {
                        $(".place-pickup-order").prop("disabled", false);
                ...
                } else if (($(".contact-fname").val().trim().length > 1 && $(".contact-fname").val().trim().length <= 30) &&
                    ($(".contact-lname").val().trim().length > 1 && $(".contact-lname").val().trim().length <= 30) &&
                    telephoneRegex.test($(".contact-number").val()) &&
                    regex.test($(".contact-email").val()) && zipCodeFieldValidation && nameFieldValidation) {
                    $(".place-pickup-order").prop("disabled", false);
                    $(".place-pickup-order").removeClass("place-pickup-order-disabled");
                    expressCheckoutShiptoBuilder.enabledReviewButtonForPickup();
                } else {
                ...
                //First and Last Name special character validation D-193257
                if (isD193257ToggleEnable &&
                    (nameRegex.test($(".alternate-fname").val().trim())) ||
                    (nameRegex.test($(".alternate-lname").val().trim()))
                ) {
                    nameFieldValidation = false;
                }
                if (($(".contact-fname").val().trim().length > 1 && $(".contact-fname").val().trim().length <= 30) &&
                    ($(".contact-lname").val().trim().length > 1 && $(".contact-lname").val().trim().length <= 30) &&
                    telephoneRegex.test($(".contact-number").val()) &&
                    regex.test($(".contact-email").val()) &&
                    ($(".alternate-fname").val().trim().length > 1 && $(".alternate-fname").val().trim().length <= 30) &&
                    ($(".alternate-lname").val().trim().length > 1 && $(".alternate-lname").val().trim().length <= 30) &&
                    telephoneRegex.test($(".alternate-number").val()) &&
                    regex.test($(".alternate-email").val()) && nameFieldValidation
                ) {
                    $(".place-pickup-order").prop("disabled", false);
                    $(".place-pickup-order").removeClass("place-pickup-order-disabled");
                    expressCheckoutShiptoBuilder.enabledReviewButtonForPickup();
        ...
        validateAlternateForm: function (isAlternate) {
            let regex = /^([_\-\.0-9a-zA-Z]+)@([_\-\.0-9a-zA-Z]+)\.([a-zA-Z]){2,7}$/;
            let nameFieldValidation = true;
            //First and Last Name special character validation D-193257
            if (isD193257ToggleEnable &&
            ...
            if (isAlternate &&
                ($(".alternate-fname").val().length > 1 || $(".alternate-fname").val().length <= 30) &&
                ($(".alternate-lname").val().length > 1 || $(".alternate-lname").val().length <= 30) &&
                $(".alternate-number").val().length >= 10 &&
                regex.test($(".alternate-email").val()) && nameFieldValidation) {
                $(".place-pickup-order").prop("disabled", false);
                $(".place-pickup-order").removeClass("place-pickup-order-disabled");
                expressCheckoutShiptoBuilder.enabledReviewButtonForPickup();
        ...
        shippingInformationValidation: function () {
            ...
            if (!explorersRestrictedAndRecommendedProduction) {
                if ($('input:radio[name="production-location"]').length > 0 && !$('input:radio[name="production-location"]').is(':checked')) {
                    messageContainer.addErrorMessage({
                        message: $t(
                            "Choose a production location first."
                        ),
                    });
                    return false;
                }
        ...
        isApplyShippingAccountNumber: function (fedExAccountNumber, applyRemove) {
            ...
            let firstname = $(".form-shipping-address input[name^='firstname']").val();
            let lastname = $(".form-shipping-address input[name^='lastname']").val();
            let email = $(".form-shipping-address input[name^='custom_attributes[email_id]']").val();
            let telephone = $(".form-shipping-address input[name^='telephone']").val();
            if (typeof (telephone) != 'undefined') {
                telephone = $(".form-shipping-address input[name^='telephone']").val().replace(/\D/g, '');
            }
            ...
            if (regionCode != 'undefined' && regionCode != '') {
                ...
                $.ajax({
                    ...
                    var NetTotalAmount = priceFormatWithCurrency(calculate("netAmount"));
                    var cartPrice = priceFormatWithCurrency(calculate("taxableAmount"));
                    ...
                    //handle estimated shipping total display
                    shippingFormAdditionalScript.handleEstimatedShippingTotal(estimatedShippingTotal);
                    if (!isCustomerShippingAccount3PEnabled && (estimatedShippingTotal === "TBD" || estimatedShippingTotal === "$0.00") && self.isMixedQuote()) {
                        $(".shipping-disclaimer-container").addClass('hide');
                    }
                    ...
                    if (applyRemove === 'apply') {
                        ...
                        $("#addFedExAccountNumberButton").prop("disabled", false);
                        document.getElementById("fedExAccountNumber").value = "";
                        $("#fedExAccountNumber").prop("disabled", false);
        ...
        canShowSignatureMessage: function () {
            if (isSdeStore === true && this.rates().length) {
                return true;
            }
        ...
        setAccordianBasedOnShipping: function () {
            ...
            if (this.isFullMarketplaceQuote()) {
                ...
                this.enableEarlyShippingAccountIncorporation(isCustomerShippingAccount3PEnabled);
        ...
        getSdeDirectSignatureMessage: function () {
            var signatureMessage = null;
            if (typeof window.checkoutConfig.sde_signature_message != "undefined") {
                signatureMessage = window.checkoutConfig.sde_signature_message;
            }
        ...
        hideremoveShippingAccountNumberwhennull: function () {
            ...
            if (isFclCustomer && this.isgenerateShippingAccountExecuted() < 1) {
                fclShippingAccountList.generateShippingAccountListHtml();
                var classLength = document.getElementsByClassName('shipping-account-list-container').length;
        ...
        deliveryOptionsStorePickup: ko.observable(window.checkoutConfig.is_pickup),
        ...
        handleSDEstorePickupVisibility: function () {
            let flag = true,
                preferredDelivery = profileSessionBuilder.getPreferredDeliveryMethod();
            ...
            if (isSdeStore) {
                ...
                if (!quote.isVirtual() && this.deliveryOptionsStorePickup()) {
                    if (window.location.hash != "#step_code" &&
        ...
        getShippingStepKoTemplate: function (filePath = '') {
            const shippingStepTemplatePath = "Fedex_Delivery/checkout/shipping_step/";
            return shippingStepTemplatePath + filePath;
        },
        ...
        getDeliveryMethodsTooltip: function () {
            return window.checkoutConfig.checkoutDeliveryMethodsTooltip.toString();
        },
