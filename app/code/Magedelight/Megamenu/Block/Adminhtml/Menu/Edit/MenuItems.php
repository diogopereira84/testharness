...
class MenuItems extends \Magento\Backend\Block\Template
{
    ...
    public function genereateBackendTree($items, $menuItemId)
    {
        ...
        foreach ($items as $item) {
            ...
            $itemOutput .= '<li class="dd-item col-m-12" data-id="' . $menuItemId . '" data-name="' . $itemName . '" data-type="' . $currentItem->getItemType() . '" data-objectid="' . $currentItem->getObjectId() . '" data-link="' . $currentItem->getItemLink() . '" data-verticalmenu="' . $currentItem->getCategoryVerticalMenu() . '" data-verticalmenubg="' . $currentItem->getCategoryVerticalMenuBg() . '" font-icon="' . $currentItem->getItemFontIcon() . '" animation-field="' . $currentItem->getAnimationOption() . '" item-class="' . $currentItem->getItemClass() . '" vertical-cat-sortby="' . $currentItem->getVerticalCatSortby() . '" vertical-cat-sortorder="' . $currentItem->getVerticalCatSortorder() . '" vertical-cat-level="' . $currentItem->getVerticalCatLevel() . '" data-cat="' . $categoryDisplay . '"><button class="cf removebtn btn right" href="#" type="button">Remove </button><a class="right collapse linktoggle">Collapse</a><a class="right expand linktoggle">Expand</a><div class="dd-handle">' . $itemName . "<span class='right'>(" . $this->helper->getMenuName($currentItem->getItemType()) . ")</span>" . '</div><div class="item-information col-m-12">' . $itemText . $itemColumns . '<div class="cf"></div></div>';
